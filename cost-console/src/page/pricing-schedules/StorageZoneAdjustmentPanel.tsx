...
export function StorageZoneAdjustmentPanel({ canPublish, zoneCode }: StorageZoneAdjustmentPanelProps) {
  ...
  const adjustments = useMemo(() => (result?.adjustments ?? []).map((adjustment) => {
    ...
    try {
      ...
      // API emits signed BIGINT decimal strings; corrupt responses remain explicit.
    }
  ...
  const publish = async () => {
    ...
    try {
      ...
      toast.success(`Storage Zone adjustment version ${published.version_number} published`);
      await load();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Storage Zone price adjustment publish failed');
      await load();
    } finally {
      setPublishing(false);
    }
  ...
  return (
    <div className="space-y-4 rounded-lg border border-cyan-900/50 bg-cyan-950/10 p-4">
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded border border-slate-800 bg-slate-950/50 p-3">
              ...
              <p className={`mt-2 text-lg font-bold ${current ? 'text-emerald-300' : 'text-slate-200'}`}>{current?.percentage ?? '100.00%'}</p>
              <p className="font-mono text-[10px] text-slate-400">{current ? `${current.multiplier_numerator} / ${current.multiplier_denominator} · v${current.version_number}` : 'GLOBAL rate applies at 1 / 1'}</p>
            </div>
            <div className="rounded border border-slate-800 bg-slate-950/50 p-3">
              <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Next scheduled</p>
              {upcoming[0] ? <><p className="mt-2 text-lg font-bold text-blue-300">{upcoming[0].percentage}</p><p className="text-[10px] text-slate-400">v{upcoming[0].version_number} · {new Date(upcoming[0].effective_from).toISOString().replace('T', ' ').replace('Z', ' UTC')}</p></> : <p className="mt-2 text-xs text-slate-500">No future adjustment scheduled.</p>}
            </div>
          </div>

          {canPublish ? (
            <div className="space-y-3 rounded border border-cyan-900/40 bg-slate-950/30 p-3">
              <div><p className="text-xs font-bold text-slate-200">Publish immutable Zone version</p><p className="mt-1 text-[10px] text-slate-500">Expected latest version: {latest?.version_number ?? 0}. Session proof is requested when publishing.</p></div>
              <div className="grid gap-2 sm:grid-cols-2">
                <label className="text-[10px] text-slate-400">Multiplier numerator (BIGINT string)<input type="text" inputMode="numeric" pattern="[0-9]*" value={multiplierNumerator} onChange={(event) => setMultiplierNumerator(event.target.value)} className="mt-1 w-full rounded border border-slate-700 bg-slate-950 px-2 py-1.5 font-mono text-xs text-white" /></label>
                <label className="text-[10px] text-slate-400">Multiplier denominator (BIGINT string)<input type="text" inputMode="numeric" pattern="[0-9]*" value={multiplierDenominator} onChange={(event) => setMultiplierDenominator(event.target.value)} className="mt-1 w-full rounded border border-slate-700 bg-slate-950 px-2 py-1.5 font-mono text-xs text-white" /></label>
                <label className="text-[10px] text-slate-400">Effective from (UTC+0)<input type="datetime-local" step={60} value={effectiveFrom} onChange={(event) => setEffectiveFrom(event.target.value)} className="mt-1 w-full rounded border border-slate-700 bg-slate-950 px-2 py-1.5 text-xs text-white" /></label>
                <label className="text-[10px] text-slate-400">Change reason<input value={changeReason} maxLength={2000} onChange={(event) => setChangeReason(event.target.value)} className="mt-1 w-full rounded border border-slate-700 bg-slate-950 px-2 py-1.5 text-xs text-white" /></label>
              </div>
              <button type="button" disabled={publishing} onClick={() => void publish()} className="inline-flex items-center gap-2 rounded bg-cyan-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50"><Check size={14} /> {publishing ? 'Publishing…' : 'Publish Zone adjustment'}</button>
            </div>
          ) : <div className="flex items-start gap-2 rounded border border-amber-900/40 bg-amber-950/20 p-3 text-[11px] text-amber-200"><AlertTriangle size={14} className="mt-0.5 shrink-0" />Read-only Zone history. Publishing requires pricing-schedule permission and session proof.</div>}
