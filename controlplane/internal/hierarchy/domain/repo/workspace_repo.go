...
type TenantWorkspaceRepository interface {
	...
	Create(ctx context.Context, workspace coreEntity.Workspace) (*coreEntity.Workspace, error)
	...
	ListAllByTenant(ctx context.Context, tenantID uuid.UUID) ([]*coreEntity.Workspace, error)
	
	// ListByTenantAndIDs lấy các workspace thuộc Tenant cụ thể theo danh sách IDs được duyệt
	ListByTenantAndIDs(ctx context.Context, tenantID uuid.UUID, ids []uuid.UUID) ([]*coreEntity.Workspace, error)
	...
	Update(ctx context.Context, workspace coreEntity.Workspace) (*coreEntity.Workspace, error)
