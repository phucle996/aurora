...
type TenantWorkspaceRepository interface {
	...
	Create(ctx context.Context, workspace entity.TenantWorkspace) (*entity.TenantWorkspace, error)

	// GetByID tìm kiếm thông tin chi tiết của một Workspace theo ID
	GetByID(ctx context.Context, id uuid.UUID) (*entity.TenantWorkspace, error)

	// ListAllByTenant lấy toàn bộ workspace thuộc Tenant (cho Tenant Admin/Owner)
	ListAllByTenant(ctx context.Context, tenantID uuid.UUID) ([]*entity.TenantWorkspace, error)

	// ListByTenantAndIDs lấy các workspace thuộc Tenant cụ thể theo danh sách IDs được duyệt
	ListByTenantAndIDs(ctx context.Context, tenantID uuid.UUID, ids []uuid.UUID) ([]*entity.TenantWorkspace, error)

	// ListCatalogAllByTenant lấy catalog toàn bộ workspace thuộc Tenant trong Zone cụ thể
	ListCatalogAllByTenant(ctx context.Context, tenantID uuid.UUID, zoneID uuid.UUID) ([]entity.WorkspaceCatalog, error)

	// ListCatalogByTenantAndIDs lấy catalog các workspace thuộc Tenant cụ thể theo danh sách IDs được duyệt trong Zone
	ListCatalogByTenantAndIDs(ctx context.Context, tenantID uuid.UUID, zoneID uuid.UUID, ids []uuid.UUID) ([]entity.WorkspaceCatalog, error)

	// Update cập nhật thông tin của workspace
	Update(ctx context.Context, workspace entity.TenantWorkspace) (*entity.TenantWorkspace, error)
...
type PersonalWorkspaceRepository interface {
	...
	Create(ctx context.Context, workspace entity.PersonalWorkspace) (*entity.PersonalWorkspace, error)

	// GetByID tìm kiếm thông tin chi tiết của một Workspace cá nhân theo ID
	GetByID(ctx context.Context, id uuid.UUID) (*entity.PersonalWorkspace, error)

	// ListByOwner lấy toàn bộ workspace cá nhân do user sở hữu (tránh orphan workspace)
	ListByOwner(ctx context.Context, ownerID uuid.UUID) ([]*entity.WorkspacePersonalListItem, error)

	// ListCatalogByOwner lấy catalog toàn bộ workspace cá nhân do user sở hữu trong Zone cụ thể
	ListCatalogByOwner(ctx context.Context, ownerID uuid.UUID, zoneID uuid.UUID) ([]entity.WorkspaceCatalog, error)

	// Update cập nhật thông tin của workspace cá nhân
	Update(ctx context.Context, workspace entity.PersonalWorkspace) (*entity.PersonalWorkspace, error)
