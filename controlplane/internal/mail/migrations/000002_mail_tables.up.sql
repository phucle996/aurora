...
CREATE TABLE IF NOT EXISTS mail_endpoints (
    ...
    provider VARCHAR(100) NOT NULL, -- Nhà cung cấp dịch vụ mail (smtp, sendgrid, mailgun)
    connection_config BYTEA NOT NULL, -- Cấu hình kết nối đã được mã hóa phong bì (Envelope Encryption - AES-256-GCM)
...
COMMENT ON TABLE mail_endpoints IS 'Stores connection configurations to mail senders (SMTP/SaaS). Sensitive parts must be fully encrypted.';
...
COMMENT ON COLUMN mail_endpoints.provider IS 'Physical delivery provider technology (smtp, sendgrid, mailgun).';
COMMENT ON COLUMN mail_endpoints.connection_config IS 'Encrypted binary block containing credentials (SMTP host/user/pass, API tokens). Encrypted using AES-256-GCM envelope key.';
