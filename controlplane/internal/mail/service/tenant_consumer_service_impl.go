...
func NewTenantConsumerService(repo mailRepoInterface.TenantConsumerRepository, redisJob *goredis.Client) mailSvcInterface.TenantConsumerService {
	return &tenantConsumerServiceImpl{repo: repo, redisJob: redisJob}
}
...
func (s *tenantConsumerServiceImpl) WatchConsumerRuntime(ctx context.Context, req *mailEntity.WatchTenantConsumerRuntime) (*mailEntity.WatchTenantConsumerRuntime, error) {
	...
	if s.redisJob == nil {
		return nil, fmt.Errorf("mail tenant runtime watch: redis job unavailable: %w", mailTaxonomy.ErrRuntimeUnavailable)
	}
