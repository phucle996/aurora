...
type PersonalTemplateHandler struct {
	svc mailSvcInterface.PersonalTemplateService
}

func NewPersonalTemplateHandler(svc mailSvcInterface.PersonalTemplateService) *PersonalTemplateHandler {
	return &PersonalTemplateHandler{svc: svc}
}

func (h *PersonalTemplateHandler) Create(c *gin.Context) {
	const op = "mail.personal.template.create"
	...
	template, err := h.svc.CreateTemplate(ctx, &mailEntity.PersonalTemplate{ActorUserID: actorID, WorkspaceID: &workspaceID, ZoneID: zoneID, IdempotencyKey: req.IdempotencyKey, Name: req.Name, SubjectTemplate: req.SubjectTemplate, TextTemplate: req.TextTemplate, HTMLTemplate: req.HTMLTemplate, VariableSchemaJSON: req.VariableSchemaJSON})
	version := template
	if err != nil {
		switch {
...
func (h *PersonalTemplateHandler) Get(c *gin.Context) {
	const op = "mail.personal.template.get"
...
func (h *PersonalTemplateHandler) List(c *gin.Context) {
	const op = "mail.personal.template.list"
	ctx, cancel := context.WithTimeout(pkgcontext.WithOperation(c.Request.Context(), op), 5*time.Second)
	defer cancel()
	actorID, ok := pkgcontext.GetUserID(c, op)
	if !ok {
...
func (h *PersonalTemplateHandler) ListVersions(c *gin.Context) {
	const op = "mail.personal.template.list_versions"
	ctx, cancel := context.WithTimeout(pkgcontext.WithOperation(c.Request.Context(), op), 5*time.Second)
	defer cancel()
	actorID, ok := pkgcontext.GetUserID(c, op)
	if !ok {
...
func (h *PersonalTemplateHandler) PublishVersion(c *gin.Context) {
	const op = "mail.personal.template.publish_version"
	ctx, cancel := context.WithTimeout(pkgcontext.WithOperation(c.Request.Context(), op), 15*time.Second)
	defer cancel()
	actorID, ok := pkgcontext.GetUserID(c, op)
	if !ok {
	...
	template, err := h.svc.PublishTemplateVersion(ctx, &mailEntity.PersonalTemplate{ActorUserID: actorID, WorkspaceID: &workspaceID, ZoneID: zoneID, TemplateID: templateID, ExpectedRevision: req.ExpectedRevision, SubjectTemplate: req.SubjectTemplate, TextTemplate: req.TextTemplate, HTMLTemplate: req.HTMLTemplate, VariableSchemaJSON: req.VariableSchemaJSON})
	version := template
	if err != nil {
		switch {
		case errors.Is(err, mailTaxonomy.ErrInvalidArgument), errors.Is(err, mailTaxonomy.ErrTemplateSyntax):
...
func (h *PersonalTemplateHandler) Archive(c *gin.Context) {
	const op = "mail.personal.template.archive"
	...
	err := h.svc.ArchiveTemplate(ctx, &mailEntity.PersonalTemplate{ActorUserID: actorID, WorkspaceID: &workspaceID, ZoneID: zoneID, TemplateID: templateID, ExpectedRevision: req.ExpectedRevision})
	if err != nil {
		switch {
		case errors.Is(err, mailTaxonomy.ErrInvalidArgument), errors.Is(err, mailTaxonomy.ErrTemplateSyntax):
			logger.HandlerWarn(c, op, err, "invalid mail request")
