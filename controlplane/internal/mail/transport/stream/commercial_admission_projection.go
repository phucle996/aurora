...
const (
	mailCommercialAdmissionStream = "billing.commercial.admission.mail.changed.v1"
	mailCommercialAdmissionGroup  = "controlplane-mail-commercial-admission-v1"
)

type CommercialAdmissionProjectionConsumer struct {
	rds     *goredis.Client
	service mailSvcInterface.CommercialAdmissionProjectionService
...
func NewCommercialAdmissionProjectionConsumer(
	...
	return &CommercialAdmissionProjectionConsumer{rds: rds, service: service, cancel: func() {}}
}

func (c *CommercialAdmissionProjectionConsumer) Start() error {
	ctx, cancel := context.WithCancel(context.Background())
	if err := c.rds.XGroupCreateMkStream(ctx, mailCommercialAdmissionStream, mailCommercialAdmissionGroup, "0").Err(); err != nil && !strings.Contains(err.Error(), "BUSYGROUP") {
		cancel()
		return fmt.Errorf("create Mail commercial admission consumer group: %w", err)
	}
	c.cancel = cancel
	c.wg.Add(1)
	go func() {
...
func (c *CommercialAdmissionProjectionConsumer) run(ctx context.Context) {
	consumer := "mail-admission-" + uuid.NewString()
	process := func(messages []goredis.XMessage) {
		for _, message := range messages {
			...
			var event mailProto.CommercialAdmissionChangedV1
			if len(payload) == 0 || len(payload) > 64*1024 || proto.Unmarshal(payload, &event) != nil {
				logger.SysWarn("mail.commercial_admission.invalid", "commercial admission event rejected")
			...
			if envelopeErr != nil || eventErr != nil || ownerErr != nil ||
				...
				logger.SysWarn("mail.commercial_admission.invalid", "commercial admission transport rejected")
				c.ack(ctx, message.ID)
				continue
			}
			err := c.service.Apply(ctx, &mailEntity.CommercialAdmissionProjectionCommand{
			...
			if err != nil {
				if errors.Is(err, mailTaxonomy.ErrInvalidCommercialAdmissionProjection) {
					logger.SysWarn("mail.commercial_admission.invalid", "commercial admission event rejected")
					c.ack(ctx, message.ID)
				}
				continue
	...
	for ctx.Err() == nil {
		claimed, _, claimErr := c.rds.XAutoClaim(ctx, &goredis.XAutoClaimArgs{
			Stream: mailCommercialAdmissionStream, Group: mailCommercialAdmissionGroup,
		...
		streams, err := c.rds.XReadGroup(ctx, &goredis.XReadGroupArgs{
			Group: mailCommercialAdmissionGroup, Consumer: consumer,
			Streams: []string{mailCommercialAdmissionStream, ">"}, Count: 64, Block: 5 * time.Second,
		}).Result()
		if err != nil {
			if errors.Is(err, goredis.Nil) || ctx.Err() != nil {
				continue
			}
			logger.SysWarn("mail.commercial_admission.read", err.Error())
			continue
		}
...
func (c *CommercialAdmissionProjectionConsumer) ack(ctx context.Context, messageID string) {
	_, _ = c.rds.TxPipelined(ctx, func(pipe goredis.Pipeliner) error {
		pipe.XAck(ctx, mailCommercialAdmissionStream, mailCommercialAdmissionGroup, messageID)
		pipe.XDel(ctx, mailCommercialAdmissionStream, messageID)
		return nil
	})
}
