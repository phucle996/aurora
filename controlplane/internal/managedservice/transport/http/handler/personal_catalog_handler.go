...
type PersonalCatalogHandler struct {
	service managedservice.PersonalCatalogService
}

func NewPersonalCatalogHandler(service managedservice.PersonalCatalogService) *PersonalCatalogHandler {
	return &PersonalCatalogHandler{service: service}
}

func (h *PersonalCatalogHandler) ListPersonalCatalog(c *gin.Context) {
	const op = "managedservice.personal_catalog.list"
	...
	request := dto.ListPersonalCatalogQuery{Limit: strings.TrimSpace(c.Query("limit")), Cursor: strings.TrimSpace(c.Query("cursor"))}
	limit := 50
	if request.Limit != "" {
