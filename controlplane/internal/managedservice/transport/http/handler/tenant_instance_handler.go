...
func (h *TenantInstanceHandler) CreateTenantInstance(c *gin.Context) {
	const op = "managedservice.tenant_instance.create"
	actorUserID, ok := pkgcontext.GetUserID(c, op)
	if !ok {
		return
	}
	...
	var request dto.CreateTenantInstanceRequest
	decoder := json.NewDecoder(c.Request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&request); err != nil {
		apires.RespondBadRequest(c, "invalid request")
	...
	intentSum := sha256.Sum256(append([]byte(workspaceID.String()+":"+request.Code+":"+blueprintRevisionID.String()+":"), parameters...))
	ctx, cancel := context.WithTimeout(pkgcontext.WithOperation(c.Request.Context(), op), 10*time.Second)
	defer cancel()
	result, err := h.service.CreateTenantInstance(ctx, &entity.CreateTenantInstance{ActorUserID: actorUserID, TenantID: tenantID, WorkspaceID: workspaceID, ZoneID: zoneID, Code: request.Code, Name: request.Name, BlueprintRevisionID: blueprintRevisionID, InputSchemaSHA256: inputSchemaHash, Parameters: parameters, InputSHA256: inputSum[:], DesiredSpecSHA256: desiredSum[:], CreateIntentSHA256: intentSum[:]})
	if err != nil {
		switch {
		case errors.Is(err, taxonomy.ErrNotFound):
...
func (h *TenantInstanceHandler) ResizeTenantInstance(c *gin.Context) {
	const op = "managedservice.tenant_instance.resize"
	actorUserID, ok := pkgcontext.GetUserID(c, op)
	if !ok {
		return
	}
	...
	var request dto.ResizeTenantInstanceRequest
	decoder := json.NewDecoder(c.Request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&request); err != nil {
		apires.RespondBadRequest(c, "invalid request")
	...
	ctx, cancel := context.WithTimeout(pkgcontext.WithOperation(c.Request.Context(), op), 10*time.Second)
	defer cancel()
	result, err := h.service.ResizeTenantInstance(ctx, &entity.ResizeTenantInstance{ActorUserID: actorUserID, TenantID: tenantID, WorkspaceID: workspaceID, ZoneID: zoneID, Code: code, ExpectedGeneration: request.ExpectedGeneration, Parameters: parameters, InputSHA256: inputSum[:], DesiredSpecSHA256: desiredSum[:]})
	if err != nil {
		switch {
		case errors.Is(err, taxonomy.ErrNotFound):
			apires.RespondNotFound(c, "managed service instance not found")
...
func (h *TenantInstanceHandler) DeleteTenantInstance(c *gin.Context) {
	const op = "managedservice.tenant_instance.delete"
	actorUserID, ok := pkgcontext.GetUserID(c, op)
	if !ok {
		return
	}
	...
	var request dto.DeleteTenantInstanceRequest
	decoder := json.NewDecoder(c.Request.Body)
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&request); err != nil {
		apires.RespondBadRequest(c, "invalid request")
	...
	ctx, cancel := context.WithTimeout(pkgcontext.WithOperation(c.Request.Context(), op), 10*time.Second)
	defer cancel()
	result, err := h.service.DeleteTenantInstance(ctx, &entity.DeleteTenantInstance{ActorUserID: actorUserID, TenantID: tenantID, WorkspaceID: workspaceID, ZoneID: zoneID, Code: code, ExpectedGeneration: request.ExpectedGeneration})
	if err != nil {
		switch {
		case errors.Is(err, taxonomy.ErrNotFound):
			apires.RespondNotFound(c, "managed service instance not found")
...
func (h *TenantInstanceHandler) RetryTenantInstance(c *gin.Context) {
	const op = "managedservice.tenant_instance.retry"
	actorUserID, ok := pkgcontext.GetUserID(c, op)
	if !ok {
		return
	}
	...
	ctx, cancel := context.WithTimeout(pkgcontext.WithOperation(c.Request.Context(), op), 10*time.Second)
	defer cancel()
	result, err := h.service.RetryTenantInstance(ctx, &entity.RetryTenantInstance{ActorUserID: actorUserID, TenantID: tenantID, WorkspaceID: workspaceID, ZoneID: zoneID, Code: code, OperationID: operationID})
	if err != nil {
		switch {
		case errors.Is(err, taxonomy.ErrNotFound):
			apires.RespondNotFound(c, "managed service operation not found")
