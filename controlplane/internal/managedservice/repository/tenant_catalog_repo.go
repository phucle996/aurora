...
func NewTenantCatalogRepository(db *pgxpool.Pool, managedSchema, hierarchySchema string) managedrepo.TenantCatalogRepository {
	return &tenantCatalogRepository{db: db, managedSchema: managedSchema, hierarchySchema: hierarchySchema}
}

func (r *tenantCatalogRepository) ListTenantCatalog(ctx context.Context, in *entity.ListTenantCatalog) (*entity.TenantCatalogPage, error) {
	// [COMMENT]: Tenant authorization was decided by the upstream middleware;
	// this CTE enforces only the durable tenant/workspace/Zone ownership
	// invariant and deliberately does not re-evaluate role membership.
	...
	defer rows.Close()
	items := make([]entity.TenantCatalogItem, 0, in.Limit+1)
	for rows.Next() {
		var item entity.TenantCatalogItem
		if err := rows.Scan(
	...
	page := &entity.TenantCatalogPage{Items: items}
