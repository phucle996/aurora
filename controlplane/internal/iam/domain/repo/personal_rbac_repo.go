...
// [COMMENT]: PersonalRbacRepository định nghĩa interface quản lý RBAC ở cấp độ platform / personal (quản trị toàn cục)
type PersonalRbacRepository interface {
	...
	// [COMMENT]: CreateRole tạo một vai trò hệ thống mới và map danh sách permissions kèm kiểm tra sở hữu tập con quyền của caller
	...
	// [COMMENT]: UpdateRole cập nhật thông tin vai trò platform cùng danh sách permissions được gán có kiểm tra cấp bậc caller level, trả về danh sách user ID bị ảnh hưởng
	UpdateRole(ctx context.Context, callerUserID uuid.UUID, callerLevel uint8, input *iamEntity.UpdateRoleInput) ([]uuid.UUID, error)
}
