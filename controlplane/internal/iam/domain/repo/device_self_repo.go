...
type DeviceSelfRepository interface {
	...
	// [COMMENT]: EvictDevicesByClientDeviceIDs thu hồi hàng loạt thiết bị của một user theo danh sách client_device_id và xóa refresh token tương ứng
	EvictDevicesByClientDeviceIDs(ctx context.Context, userID uuid.UUID, clientDeviceIDs []string) error
}
