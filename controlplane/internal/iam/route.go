...
func RegisterRoutes(router *gin.Engine, module *IAMModule) {
	...
	// ------------------------------------------------------------------------
	// 👤 CURRENT USER SELF API (Dành cho ngữ cảnh /api/v1/me)
	// ------------------------------------------------------------------------
	// [COMMENT]: 1.2) Lấy thông tin cá nhân (Profile) của user hiện tại
	router.GET("/api/v1/me/iam/profile/read",
		module.UserHandler.GetMyProfile,
	)

	router.PATCH("/api/v1/me/iam/profile",
		module.UserHandler.UpdateMyProfile,
	)

	router.GET("/api/v1/me/iam/social-link",
	...
	router.DELETE("/api/v1/me/critical/iam/social-link/:provider",
		middleware.RequireSessionProof(),
		module.UserHandler.UnlinkMySocialLink,
	)

	router.GET("/api/v1/me/iam/mfa",
		module.MfaHandler.GetMyMfa,
	)

	router.POST("/api/v1/me/critical/iam/mfa/setup/start",
		middleware.RequireSessionProof(),
		module.MfaHandler.StartMyMfaSetup,
	)

	router.POST("/api/v1/me/critical/iam/mfa/setup/:setup_id/confirm",
		middleware.RequireSessionProof(),
		module.MfaHandler.ConfirmMyMfaSetup,
	)

	router.POST("/api/v1/me/critical/iam/mfa/recovery/regenerate",
		middleware.RequireSessionProof(),
		module.MfaHandler.RegenerateMyRecoveryCodes,
	)

	router.DELETE("/api/v1/me/critical/iam/mfa",
		middleware.RequireSessionProof(),
	...
	// ------------------------------------------------------------------------
	// 👑 PERSONAL API (Dành cho Quản trị viên hệ thống & Người dùng cá nhân)
	// ------------------------------------------------------------------------
	...
	router.PUT("/api/v1/personal/critical/iam/users/:id/status",
		middleware.RequireSessionProof(),
	...
	router.PUT("/api/v1/personal/critical/iam/users/:id/password",
		middleware.RequireSessionProof(),
		middleware.Authorize("iam:users:manage", module.L1Registry, "2"),
	...
	router.GET("/api/v1/personal/iam/users/:id/mfa",
	...
	// ------------------------------------------------------------------------
	// 🏢 TENANT API (Dành cho ngữ cảnh Tenant, quản trị viên Tenant)
	// ------------------------------------------------------------------------
