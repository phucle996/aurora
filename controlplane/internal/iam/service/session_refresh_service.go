...
func (s *SessionRefreshService) CreateUserOpaqueSession(ctx context.Context, userID uuid.UUID, deviceID uuid.UUID) (string, time.Time, error) {
	// 1. Tạo token đục ngẫu nhiên có độ dài an toàn cao (high entropy)
	rawRefresh, err := security.GenerateToken(43)
	if err != nil {
		return "", time.Time{}, fmt.Errorf("session refresh: failed to generate refresh token: %w", err)
	}

	// 2. Tạo UUID v7 cho ID và Family ID (phục vụ mục đích truy vết tuyến tính và rotation)
	...
	// 3. Chuẩn bị struct thực thể refresh token
	...
	// 4. Ghi trực tiếp xuống DB PostgreSQL thông qua Repository của RefreshToken
