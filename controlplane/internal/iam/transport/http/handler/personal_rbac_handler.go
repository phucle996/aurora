...
// [COMMENT]: PersonalRbacHandler xử lý các HTTP endpoints quản trị vai trò ở cấp độ personal / platform
type PersonalRbacHandler struct {
	personalRbacSvc iamSvcInterface.PersonalRbacService
}

// [COMMENT]: NewPersonalRbacHandler khởi tạo HTTP handler cho Personal RBAC
func NewPersonalRbacHandler(personalRbacSvc iamSvcInterface.PersonalRbacService) *PersonalRbacHandler {
	return &PersonalRbacHandler{personalRbacSvc: personalRbacSvc}
}

// [COMMENT]: AssignUserRole gán role hệ thống cho user
func (h *PersonalRbacHandler) AssignUserRole(c *gin.Context) {
	...
	err = h.personalRbacSvc.AssignUserRole(ctx, callerLevel, userUUID, roleUUID)
	if err != nil {
...
func (h *PersonalRbacHandler) ListRolesPlatform(c *gin.Context) {
	...
	roles, err := h.personalRbacSvc.ListPlatformRoles(ctx, callerLevel)
	if err != nil {
		logger.HandlerError(c, op, err)
...
func (h *PersonalRbacHandler) CreateRole(c *gin.Context) {
	...
	err := h.personalRbacSvc.CreateRole(ctx, callerUserID, role, permUUIDs)
	if err != nil {
		logger.HandlerError(c, op, err)
		if errors.Is(err, iamTaxonomy.ErrActionNotAllowed) {
...
func (h *PersonalRbacHandler) GetUserRolesPlatform(c *gin.Context) {
	...
	role, err := h.personalRbacSvc.GetUserRoleDetails(ctx, targetUserID, int32(callerLevel))
	if err != nil {
...
func (h *PersonalRbacHandler) ListPermissions(c *gin.Context) {
	...
	perms, err := h.personalRbacSvc.ListPermissions(ctx, callerUserID)
	if err != nil {
		logger.HandlerError(c, op, err)
...
func (h *PersonalRbacHandler) DeleteRolePlatform(c *gin.Context) {
	...
	err = h.personalRbacSvc.DeleteRolePlatform(ctx, callerLevel, roleID)
	if err != nil {
		logger.HandlerError(c, op, err)
		if errors.Is(err, iamTaxonomy.ErrActionNotAllowed) {
...
func (h *PersonalRbacHandler) GetRoleDetailsPlatform(c *gin.Context) {
	...
	role, permissions, err := h.personalRbacSvc.GetRoleDetails(ctx, callerLevel, roleID)
	if err != nil {
		logger.HandlerError(c, op, err)
		if errors.Is(err, iamTaxonomy.ErrRoleNotFound) {
			apires.RespondNotFound(c, "Role not found")
...
func (h *PersonalRbacHandler) UpdateRolePlatform(c *gin.Context) {
	...
	err = h.personalRbacSvc.UpdateRole(ctx, callerUserID, callerLevel, input)
	if err != nil {
		logger.HandlerError(c, op, err)
		if errors.Is(err, iamTaxonomy.ErrRoleNotFound) {
			apires.RespondNotFound(c, "Role not found")
