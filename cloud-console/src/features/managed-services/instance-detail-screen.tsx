...
export function ManagedServiceInstanceDetailScreen() {
  ...
  const resizeMutation = useMutation({
    mutationFn: () => {
      ...
      return resizeManagedServiceInstance(code, { expected_generation: instance.generation, resources });
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: () => {
      if (!instance) throw new Error("Instance is not loaded.");
      return deleteManagedServiceInstance(code, instance.generation);
    },
    ...
    <div className="w-full pb-10 text-foreground">
      ...
      <Tabs defaultValue="overview" className="mt-6">
        ...
        <TabsContent value="overview" className="mt-5 space-y-5">
          <div className="grid gap-4 md:grid-cols-2"><div className="rounded-[6px] border border-border p-5"><p className="text-xs text-muted-foreground">Lifecycle state</p><p className="mt-2 text-lg font-semibold">{stateLabel(instance.state)}</p><p className="mt-1 text-xs text-muted-foreground">generation {instance.generation}</p></div><div className="rounded-[6px] border border-border p-5"><p className="text-xs text-muted-foreground">Latest operation</p><p className="mt-2 text-lg font-semibold">{instance.latest_operation ? stateLabel(instance.latest_operation.kind) : "None"}</p><p className="mt-1 text-xs text-muted-foreground">{instance.latest_operation ? stateLabel(instance.latest_operation.state) : "No operation recorded"}</p></div></div>
        ...
        <TabsContent value="configuration" className="mt-5 space-y-5">
          <div className="rounded-[6px] border border-border p-5"><h2 className="text-sm font-semibold">Pinned revision</h2><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><dt className="text-muted-foreground">Active revision</dt><dd className="break-all font-mono text-xs">{instance.active_revision_id ?? "Pending"}</dd><dt className="text-muted-foreground">Pending revision</dt><dd className="break-all font-mono text-xs">{instance.pending_revision_id ?? "None"}</dd><dt className="text-muted-foreground">Revision sequence</dt><dd>{instance.revision_sequence}</dd><dt className="text-muted-foreground">Metadata version</dt><dd>{instance.metadata_version}</dd></dl></div>
          {canWrite ? <div className="rounded-[6px] border border-border p-5"><h2 className="text-sm font-semibold">Resize</h2><p className="mt-1 text-xs text-muted-foreground">The pinned SRE contract defines mutable inputs. Values start empty and are never copied from protected payloads.</p>{instance.resize_contract ? <div className="mt-4 space-y-4">{resizeGroups.map((group) => { const fields = instance.resize_contract?.ui_schema.fields.filter((field) => field.group === group.key && instance.resize_contract?.input_schema.fields.some((input) => input.key === field.key && input.mutable)).sort((left, right) => left.order - right.order) ?? []; return fields.length > 0 ? <div key={group.key} className="rounded border border-border/70 p-4"><p className="text-xs font-semibold">{localizedText(group.label_i18n, locale) || group.key}</p><div className="mt-3 grid gap-4 sm:grid-cols-2">{fields.map((uiField) => { const input = instance.resize_contract?.input_schema.fields.find((candidate) => candidate.key === uiField.key); return input ? <ManagedServiceContractField key={input.key} input={input} ui={uiField} locale={locale} value={resizeValues[input.key]} onChange={(value) => setResizeValues((current) => ({ ...current, [input.key]: value }))} /> : null; })}</div></div> : null; })}<Button disabled={resizeMutation.isPending || instance.state !== "active"} onClick={() => resizeMutation.mutate()}>{resizeMutation.isPending ? "Submitting…" : "Submit resize"}</Button></div> : <p className="mt-4 text-sm text-muted-foreground">This revision does not expose a resize contract.</p>}{instance.state !== "active" ? <p className="mt-3 text-xs text-amber-600">Resize is available only while the lifecycle state is active.</p> : null}</div> : null}
        </TabsContent>
