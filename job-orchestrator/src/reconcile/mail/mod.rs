...
// The explicit envelope fields make the cross-Zone security boundary auditable
// at each callsite; hiding them in mutable shared state would weaken review.
...
pub(super) async fn publish_mail_projection_command(
    ...
    .key(format!("mail:reconcile:{{{zone_id}}}:lock"))
    .arg(generation)
