...
use crate::contracts::mail::{MailConsumerRuntimeReportBatchV1, MailConsumerRuntimeState};
            ...
            if let (Some(zone_id), Some(batch)) = (zone_id, batch) {
                ...
                if Uuid::from_slice(&batch.zone_id).ok() != Some(zone_id) {
                    ...
                    for report in batch.reports {
                        ...
                        // Every key touched by a Lua/MGET operation shares the
                        // consumer hash tag, which is mandatory on Redis Cluster.
                        let active_watch_key =
                            format!("mail:runtime:{{{consumer_id}}}:watch-active:{zone_id}");
                        let slot_key = format!(
                            "mail:runtime:{{{consumer_id}}}:slot:{scope}:{}",
                            report.instance_id
                        );
                        let slot_index_key =
                            format!("mail:runtime:{{{consumer_id}}}:slot-index:{scope}");
                        let revision_key =
                            format!("mail:runtime:{{{consumer_id}}}:revision:{scope}");
                        ...
                        let snapshot_key =
                            format!("mail:runtime:{{{consumer_id}}}:snapshot:{scope}");
                    ...
                    .await;
                        ...
                        // Watcher list and notification bridge are Central-local Shared Redis.
                        // NATS Core ends at JO; the TTL snapshot remains the recovery source.
                        ...
                        .key(format!("mail:runtime:{{{consumer_id}}}:watchers:{zone_id}"))
                        .invoke_async(&mut redis_conn)
                        .await;
                        if let Ok(watchers) = watchers {
                            ...
                            let mut notifications = redis::pipe();
                            let mut notification_count = 0_usize;
                            ...
                            if notification_count > 0 {
                                let publish_result: redis::RedisResult<()> =
                                    notifications.query_async(&mut redis_conn).await;
