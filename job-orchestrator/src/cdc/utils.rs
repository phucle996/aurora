...
pub fn parse_pg_config(url: &str) -> Result<(String, u16, String, String, String), String> {
    let pg_config = url.parse::<tokio_postgres::Config>().map_err(|e| e.to_string())?;
    
    // Lấy host đầu tiên trong danh sách hosts
    let host = match pg_config.get_hosts().first().ok_or_else(|| "No host found in database URL".to_string())? {
