...
// Trả về actor_user_id, job_topic, trace_id, resource_id phục vụ OTel và phát sự kiện real-time.
pub async fn update_outbox_record(
    ...
    Logger::sys_info(
        ...
        &format!(
            "Cập nhật trạng thái Outbox cho IAM Job: {} -> {}",
            job_uuid, status
        ),
    );
            ...
            .query_opt(
                "UPDATE iam.iam_outbox_records \
                 SET status = $1, \
                     ...
                     error_code = NULL, \
                     error_message = NULL \
				 WHERE event_id = $2::uuid AND job_topic = $3 AND status IN ('PUBLISHING', 'PUBLISHED', 'PROCESSING') \
            ...
            .query_opt(
                "UPDATE iam.iam_outbox_records \
                 SET status = $1, \
                     ...
                     error_code = $2, \
                     error_message = $3 \
				 WHERE event_id = $4::uuid AND job_topic = $5 AND status IN ('PUBLISHING', 'PUBLISHED', 'PROCESSING') \
                 RETURNING actor_user_id::text, job_topic, trace_id, resource_id",
