    ...
    let authority = tx
        .query_opt(
            "SELECT outbox.status, outbox.owner_type, outbox.zone_id, outbox.delivery_epoch, \
             ...
             JOIN managed_service.tenant_managed_service_instances instance \
    ...
    let row = if result.status == "SUCCEEDED" {
        match operation_kind.as_str() {
            "create" | "resize" => {
                tx.query_opt(
                    "WITH promoted AS ( \
                         UPDATE managed_service.tenant_managed_service_instances \
                         ...
                         WHERE id = $1 AND generation = $2 AND pending_revision_id IS NOT NULL RETURNING id \
                     ), updated_operation AS ( \
                         UPDATE managed_service.tenant_managed_service_operations \
                         SET state = 'succeeded', attempt = $3, completed_at = NOW(), \
                             last_error_code = NULL, last_sanitized_error = NULL, status_version = status_version + 1, updated_at = NOW() \
                         WHERE id = $4 AND current_command_event_id = $5 \
                           AND state IN ('accepted','dispatching','running','retrying') \
                           AND EXISTS (SELECT 1 FROM promoted) RETURNING id \
                     ) \
                     ...
                     WHERE outbox.event_id = $5 AND outbox.job_topic = $6 AND outbox.status IN ('PENDING','PROCESSING') \
            ...
            "delete" => {
                tx.query_opt(
                    "WITH fenced AS ( \
                         INSERT INTO managed_service.tenant_managed_service_deletion_fences (instance_id, operation_id, generation, zone_id, retained_until) \
                         SELECT $1, $2, $3, zone_id, GREATEST(retained_until, NOW() + interval '30 days') \
                         FROM managed_service.tenant_managed_service_operations WHERE id = $2 \
                         ON CONFLICT (instance_id, operation_id, generation) DO NOTHING RETURNING instance_id \
                     ), deleted_revisions AS ( \
                         DELETE FROM managed_service.tenant_managed_service_instance_revisions WHERE instance_id = $1 AND EXISTS (SELECT 1 FROM fenced) RETURNING id \
                     ), deleted_instance AS ( \
                         DELETE FROM managed_service.tenant_managed_service_instances WHERE id = $1 AND EXISTS (SELECT 1 FROM fenced) RETURNING id \
                     ), updated_operation AS ( \
                         UPDATE managed_service.tenant_managed_service_operations SET state = 'succeeded', attempt = $4, completed_at = NOW(), \
                             last_error_code = NULL, last_sanitized_error = NULL, status_version = status_version + 1, updated_at = NOW() \
                         WHERE id = $2 AND current_command_event_id = $5 AND state IN ('accepted','dispatching','running','retrying') \
                           AND EXISTS (SELECT 1 FROM deleted_instance) RETURNING id \
                     ) \
                     UPDATE managed_service.managed_service_outbox_records outbox SET status = 'SUCCEEDED', completed_at = NOW(), error_code = NULL, error_message = NULL, updated_at = NOW() \
                     WHERE outbox.event_id = $5 AND outbox.job_topic = $6 AND outbox.status IN ('PENDING','PROCESSING') AND EXISTS (SELECT 1 FROM updated_operation) \
                     RETURNING outbox.actor_user_id::text, outbox.job_topic, outbox.resource_id, $5::uuid",
                    &[&instance_id, &operation_id, &operation_generation, &attempt, &job_id, &job_topic],
                )
                .await?
            }
            _ => return Err(format!("unsupported tenant operation kind '{operation_kind}'").into()),
        }
    } else {
        tx.query_opt(
            "WITH updated_instance AS ( \
                 UPDATE managed_service.tenant_managed_service_instances SET observed_state = $4::managed_service.managed_service_observed_state, observed_state_version = observed_state_version + 1, \
                     observed_output = '{}'::jsonb, observed_at = NOW(), pending_revision_id = CASE WHEN $3 = 'resize' THEN NULL ELSE pending_revision_id END, updated_at = NOW() \
                 WHERE id = $1 RETURNING id \
             ), updated_operation AS ( \
                 UPDATE managed_service.tenant_managed_service_operations SET state = 'terminal_failed', attempt = $5, completed_at = NOW(), last_error_code = $6, last_sanitized_error = LEFT($7, 1024), status_version = status_version + 1, updated_at = NOW() \
                 WHERE id = $2 AND current_command_event_id = $8 AND state IN ('accepted','dispatching','running','retrying') AND EXISTS (SELECT 1 FROM updated_instance) RETURNING id \
             ) \
             UPDATE managed_service.managed_service_outbox_records outbox SET status = 'FAILED', completed_at = NOW(), error_code = $6, error_message = LEFT($7, 1024), updated_at = NOW() \
             WHERE outbox.event_id = $8 AND outbox.job_topic = $9 AND outbox.status IN ('PENDING','PROCESSING') AND EXISTS (SELECT 1 FROM updated_operation) \
