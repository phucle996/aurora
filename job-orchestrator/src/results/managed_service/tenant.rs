            ...
            "delete" => {
                tx.query_opt(
                    "WITH fenced AS ( \
                         ...
                         DELETE FROM managed_service.tenant_managed_service_instances \
                         WHERE id = $1 AND state = 'deleting' AND EXISTS (SELECT 1 FROM fenced) RETURNING id \
                     ), updated_operation AS ( \
        ...
        tx.query_opt(
            "WITH updated_instance AS ( \
                 ...
                 SET state = CASE WHEN $3 = 'resize' THEN 'active'::managed_service.managed_service_instance_state ELSE state END, \
                     pending_revision_id = CASE WHEN $3 = 'resize' THEN NULL ELSE pending_revision_id END, updated_at = NOW() \
                 ...
                 WHERE id = $4 AND current_command_event_id = $8 AND state IN ('accepted','dispatching','running','retrying') AND EXISTS (SELECT 1 FROM updated_instance) RETURNING id \
             ) \
