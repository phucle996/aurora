...
// [COMMENT]: Thực hiện Shared Redis request/reply xác thực Trinity Token của Admin.
pub async fn verify_admin_token(
    ...
    Logger::sys_info(
        "auth_service.admin",
        "Verifying admin trinity token via Shared Redis",
    );
    ...
    let res = VerifyAdminTrinityTokenResponse::decode(response_payload.as_slice())
