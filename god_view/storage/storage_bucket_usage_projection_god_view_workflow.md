...
for the UI. Bucket detail runtime telemetry is a separate assertion-gated Zone
workflow and does not participate in this settlement.
...
| `storage.personal_buckets.used_bytes` | PostgreSQL | CTE locks the target and applies only a strictly newer observation. |
| `storage.tenant_buckets.used_bytes` | PostgreSQL | Same timestamp fence for the Tenant projection. |
...
- `AURORA_ZONE_CONFIG/zone.metadata` JSON: `status`, `services` map and
  `updated_at`; this scanner consumes only `status == "active"` and
  `services.storage == true`.
- `AURORA_ZONE_CONTROL_ASSIGNMENTS/assignment.storage_scan.{shard}` JSON:
  `unit_key`, `member_id`, `assignment_epoch`, `assigned_at_unix_ms`,
  `expires_at_unix_ms`; the worker rechecks only the expected epoch and
  non-expired lease before side effects.

Neither record contains bucket identity or usage. Bucket names and byte counts
come only from MinIO in this phase.
...
## Phase 2 — JO validation and PostgreSQL projection
...
sequenceDiagram
    ...
    alt invalid fragment
        ...
        loop each bucket in this shard fragment
            alt ws bucket
                JO->>PG: UPDATE personal used_bytes behind timestamp fence
            else tn bucket
                JO->>PG: UPDATE tenant used_bytes behind timestamp fence
            end
        end
...
Bucket list and detail APIs serialize the durable byte value as fixed-point
binary MB. The directory uses explicit refetch. Bucket detail may overlay the
separate MinIO/OTel/Victoria runtime sample, but runtime loss leaves this
durable value visible.
...
| PostgreSQL update fails | Worker stops before committing current offset; the strict observation timestamp fence makes replay safe. |
