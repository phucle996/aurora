...
owner, wallet or admission header.
...
| `Cookie`, `X-Aurora-*` | Removed or ignored; never authority for this path. |
...
| Key | Owner | Rule |
|---|---|---|
| `AURORA_ZONE_ADMISSION/name/{bucket}` | Zone Control admission projection | Current resource id/name, wallet version, mode and expiry. |
| `AURORA_ZONE_ADMISSION/{resource_id}` | Zone Control admission projection | Same state keyed by immutable bucket UUID. |
...
sequenceDiagram
    ...
    E->>E: Match public route; retain AWS headers; remove cookies and spoofed x-aurora headers
...
sequenceDiagram
    ...
    E->>S: Authorized PUT with original AWS credential headers
    S->>M: Signed PUT to minio:9000
    M-->>S: S3 status, ETag and metadata
    S-->>E: Upstream response
    E-->>SDK: Streamed S3 response
```

The public Lua filter removes cookies and untrusted Aurora headers but leaves
AWS signing headers on this non-ticket route. Envoy access logging emits only
...
into a second request, and a restricted wallet blocks the next SDK request at
Phase 1.
...
- Duplicate admission events: Zone CAS keeps the highest wallet version;
