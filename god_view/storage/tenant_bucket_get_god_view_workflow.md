# Tenant Bucket Get — God View

Tenant Bucket Get is a synchronous read workflow for querying details of a specific bucket
within an enterprise workspace. Controlplane executes 3-table join verification in PostgreSQL
...
Browser calls neutral `GET /api/v1/storage/buckets/{id}`. ACR validates the Trinity tenant membership,
resolves workspace and zone context, rewrites the path to `/api/v1/tenant/storage/buckets/{id}`,
...
|---|---|---|
| `200` | `{"status": "success", "data": { "id": "...", "name": "tn-...", "workspace_id": "...", "zone_id": "...", "tenant_id": "...", "status": "READY", "capacity_quota_bytes": 107374182400, "used_bytes": 0, "versioning_enabled": false, "lifecycle_rules": [], "created_at": "...", "updated_at": "..." }, "message": "tenant bucket retrieved successfully"}` | Target tenant bucket details. |
...
sequenceDiagram
    ...
    Browser->>Envoy: GET /api/v1/storage/buckets/{id}
    ...
    ACR-->>Envoy: Ok
    Envoy->>CP: Forward GET /api/v1/tenant/storage/buckets/{id}
```
...
sequenceDiagram
    ...
    AuthMW->>Handler: Dispatch Get(c)
    Handler->>Service: GetBucketForTenant(bucketID, workspaceID, tenantID, userID, zoneID)
...
- **Input**: `bucketID` (from path `:id`), `workspaceID`, `tenantID`, `userID`, `zoneID` (from context).
- **Output**: Call to `repo.GetByID(ctx, bucketID, workspaceID, tenantID, userID, zoneID)`.
...
- **Input SQL Query**:
  ...
  WHERE b.id = $1 
    ...
    AND w.zone_id = $5;
  ```
- **Output**: Flat `TenantBucket` entity or `ErrBucketNotFound`.

#### Hop 2.4: Controlplane → Browser
- **Output**: HTTP `200 OK` JSON object.

---

## Code map

- **God View SoT**: `god_view/storage/tenant_bucket_get_god_view_workflow.md`
- **Controlplane Route**: `controlplane/internal/storage/route.go`
- **Controlplane Handler**: `controlplane/internal/storage/transport/http/handler/tenant_bucket_handler.go` (`Get`)
- **Controlplane Service**: `controlplane/internal/storage/service/tenant_bucket_service.go` (`GetBucketForTenant`)
- **Controlplane Repository**: `controlplane/internal/storage/repository/tenant_bucket_repo.go` (`GetByID`)
