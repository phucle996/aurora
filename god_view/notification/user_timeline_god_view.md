...
- `job_notifications` ghi activity và notification inbox trước khi publish
  ...
  crash là an toàn vì Scylla insert idempotent.
- `runtime` là soft-state Pub/Sub, không ghi Scylla và có thể mất khi subscriber
  reconnect; UI phải lấy snapshot/API authoritative khi cần.
...
- Redis consumer ACK chỉ sau Scylla durability; lỗi dependency giữ PEL để
  retry. Poison contract của cả job/activity được quarantine
