# Personal Mail Consumer Drain — God View
...
`/api/v1/personal/critical/mail/consumers/:id/drain`,
...
Repository rechecks the durable facts in its own CTE: `hierarchy.personal_workspaces`: id=workspace, zone_id=Zone, owner_id=actor.
...
personal resource row moves `draining → drained` in the transaction that settles
