# Tenant Mail Consumer Creation — Master God View

Workflow này là **Source of Truth (SoT) duy nhất** quản lý toàn bộ quy trình khởi tạo một Mail Consumer doanh nghiệp/tổ chức (Tenant Mail Consumer) mới trong hệ thống Aurora:
- Tiếp nhận yêu cầu tạo Consumer từ giao diện Web Console của Tenant hoặc Backend Server SDK qua route trung lập `POST /api/v1/mail/consumers`.
- Xác thực phiên làm việc, kiểm tra bằng chứng phiên (`Session Proof`), xác minh tư cách thành viên tổ chức (`Active Tenant Membership`), bảo vệ chống replay và rewrite URL sang nhánh nội bộ `/api/v1/tenant/critical/mail/consumers` tại vùng biên ACR ExtAuthz.
- Xử lý toàn bộ logic nghiệp vụ tại Controlplane: thẩm định quyền hạn RBAC `email:consumer:create` trên phạm vi Tenant Workspace, chuẩn hóa định danh và stream payload, tính mã băm `ConfigSHA256`, niêm phong mã hóa payload Outbox bằng X25519 (`jobpayload.Protector`), và thực thi giao dịch CTE/Tx nguyên tử trên PostgreSQL với **Khóa Thẩm Quyền 4 Chiều kết hợp Ràng buộc Tenant Membership**.
...
| **Controlplane Mail Service** | Kiểm tra quyền RBAC `email:consumer:create`, Stream Normalization, CTE Tx | PostgreSQL (`mail.tenant_mail_consumers`, `mail.mail_outbox_records`) |
...
| `mail.tenant_mail_templates` | PostgreSQL (Mail DB) | `SELECT id ... FOR KEY SHARE` | Khóa chia sẻ Template; ngăn chặn việc tạo consumer trỏ vào template đang bị xóa. |
| `mail.tenant_mail_consumers` | PostgreSQL (Mail DB) | `INSERT (desired_state='PAUSED', config_version=1)` | Lưu trữ thực thể Consumer của Tenant; Unique `(workspace_id, code)`. Khởi tạo luôn ở trạng thái `PAUSED`. |
...
- **JSON Request Body**:
  ```json
  {
    "code": "tenant-order-events",
    "name": "Tenant Order Notifications Consumer",
    "source_type": "kafka",
    "broker_resource_id": "0194f83a-8b1e-7d34-92c1-382a1d820050",
    "source_config_envelope": "AQIDBAU=",
    "topic": "tenant.orders.created.v1",
    "consumer_group": "tenant-mail-notifier",
    "template_id": "tenant-order-tpl",
    "template_version": 1,
...
- **ACR Kiểm tra & Phân quyền Biên**:
  ...
  2. Xác thực tính hợp lệ của Header `X-Session-Proof` chống replay và chiếm quyền phiên.
  ...
  5. **Path Rewrite**: Chuyển đổi đường dẫn công khai trung lập thành đường dẫn nội bộ nhánh Tenant:
     `POST /api/v1/mail/consumers` $\to$ `POST /api/v1/tenant/critical/mail/consumers`
...
sequenceDiagram
    ...
    ACR->>ACR: Rewrite path sang /api/v1/tenant/critical/mail/consumers
...
## ⚡ Phase 2 — Controlplane Processing & Atomic PostgreSQL CTE/Tx Mutation
...
### 1. Khóa Thẩm Quyền 4 Chiều & Ràng buộc Tenant trong CTE (4D Authority & Tenant Lock)

Mọi thao tác truy vấn và biến đổi trạng thái trong giao dịch bắt buộc phải xác lập và kiểm tra đồng thời trên **4 chiều thẩm quyền cô lập của Tenant**:

| Chiều thẩm quyền (Dimension) | Nguồn trích xuất (Source) | Ràng buộc kiểm tra trong PostgreSQL (CTE/Tx Condition) | Mục đích bảo mật (Security Invariant) |
|---|---|---|---|
| **1. Tenant ID & Membership** | Trusted Header `x-tenant-id` & `x-user-id` | `hierarchy.tenant_memberships.tenant_id = $tenant_id AND user_id = $actor_id AND status = 'active'` | Ngăn chặn việc tạo consumer khi user không còn là thành viên active của Tenant. |
...
| **3. Zone ID** | Trusted Header `x-zone-id` | `hierarchy.tenant_workspaces.zone_id = $zone_id` | Bảo đảm tài nguyên được gán đúng vùng hạ tầng vật lý của Tenant Workspace. |
| **4. Consumer Code** | Request Body / Generated UUID | `(workspace_id, code)` Unique Constraint | Đảm bảo tính duy nhất và toàn vẹn của thực thể trong phạm vi Workspace. |

### 2. Bảng Ánh Xạ Lỗi Taxonomy (Taxonomy Error Mapping Table)

Khi giao dịch cơ sở dữ liệu không thể hoàn tất hoặc không có dòng nào được chèn (`RowsAffected == 0`), tầng Repository phân loại kết quả và ánh xạ chính xác sang lỗi Taxonomy:

| Điều kiện kiểm tra thất bại (Condition) | Mã lỗi Taxonomy (Domain Error) | HTTP Status | Thông điệp phản hồi Client (Client Response) |
|---|---|---|---|
| `!authorized` (Membership/Workspace sai) | `mailTaxonomy.ErrWorkspaceNotFound` | `404 Not Found` | Workspace không tồn tại hoặc User không có tư cách thành viên active trong Tenant. |
...
sequenceDiagram
    ...
    participant H as TenantConsumerHandler.Create
    ...
    Envoy->>Mid: POST /api/v1/tenant/critical/mail/consumers
    ...
    Repo->>DB: a. Khóa Template FOR KEY SHARE & Kiểm tra Tenant Membership + Workspace Scope
    alt Template không tồn tại hoặc sai scope Tenant
        DB-->>Repo: pgx.ErrNoRows
        Repo-->>S: ErrTemplateNotFound (404)
        S-->>H: Error 404
        H-->>Envoy: 404 Not Found
    end

    Repo->>DB: b. Kiểm tra Template Outbox dở dang (PENDING/PROCESSING)
    alt Đang có thao tác trên Template
        Repo-->>S: ErrOperationInProgress (409)
        S-->>H: Error 409
        H-->>Envoy: 409 Conflict
    end

    Repo->>DB: c. INSERT INTO tenant_mail_consumers (PAUSED, v1) với EXISTS (Membership & Workspace)
    Repo->>DB: d. INSERT INTO mail_outbox_records (PENDING)

    alt Thành công trọn vẹn
        Repo->>DB: 6. COMMIT Transaction
        Repo-->>S: nil (Thành công)
        S-->>H: consumer entity
        H-->>Envoy: 201 Created (Tenant Consumer Entity)
        Envoy-->>Client: 201 Created JSON
    end
```
...
- **Thiết lập trạng thái ban đầu (`DesiredState = PAUSED`)**:
  - `ConsumerRegistry` khởi tạo thực thể Consumer của Tenant với `state = PAUSED`, `config_version = 1`, lưu trữ cấu hình `topic`, `consumer_group`, `parallelism`, `template_id`, `sender_profile_id`.
...
sequenceDiagram
    ...
    Reg-->>Ingress: 5. Đăng ký thành công (Allocated Tenant Consumer Handle)
...
- **Timeline Worker**:
  - Tiêu thụ `OperationSettledEvent` và chèn bản ghi vào bảng `timeline.workspace_events`:
    ...
    INSERT INTO timeline.workspace_events (
        ...
        gen_random_uuid(), $workspace_id, $actor_user_id, 'MAIL_CONSUMER_CREATED', 'mail_consumer', $consumer_id,
        jsonb_build_object('code', $code, 'status', 'PAUSED', 'config_version', 1, 'tenant_id', $tenant_id), NOW()
    );
    ```
- **Audit Logger**:
  - Ghi bản ghi bất biến vào `audit.system_audit_logs` phục vụ tra cứu tuân thủ doanh nghiệp:
    `INSERT INTO audit.system_audit_logs (actor_id, tenant_id, action, resource_type, resource_id, status, created_at) VALUES ($actor_user_id, $tenant_id, 'MAIL_CONSUMER_CREATE', 'mail_consumer', $consumer_id, 'SUCCESS', NOW());`
...
- **Notification Gateway & WebSocket Delivery**:
  ...
  - Đẩy thông điệp realtime qua kết nối WebSocket/SSE đang mở tới Web Console của Tenant để cập nhật Badge `PAUSED` và danh sách Consumer của Workspace.
...
sequenceDiagram
    ...
    par Ghi vết Timeline & Audit
        TimeWorker->>TimeDB: 2a. INSERT INTO timeline.workspace_events (MAIL_CONSUMER_CREATED, tenant_id)
    ...
    and
        ...
        NotiGW->>Client: 4. WebSocket Push Event (Badge PAUSED, Workspace Toast)
    end
```
...
|---|---|---|
| **User không còn là thành viên Active của Tenant** | CTE / Tx kiểm tra bảng `hierarchy.tenant_memberships` thấy `status != 'active'` $\to$ Trả về `404 Not Found` (`ErrWorkspaceNotFound`). | Quản trị viên Tenant kích hoạt lại quyền thành viên cho user trước khi thao tác. |
...
| **Template được gán không tồn tại trong Tenant Workspace** | Khóa `FOR KEY SHARE` trên template trả về `pgx.ErrNoRows` $\to$ Giao dịch bị hủy và trả về `404 Not Found` (`ErrTemplateNotFound`). | Người dùng kiểm tra lại danh sách template hợp lệ của Tenant Workspace. |
| **Template đang trong tiến trình xuất bản phiên bản mới** | Tx phát hiện có bản ghi outbox của template ở trạng thái `PENDING`/`PROCESSING` $\to$ Trả về `409 Conflict` (`ErrOperationInProgress`). | Người dùng đợi vài giây cho template publish hoàn tất rồi tạo lại. |
...
### Phase 2 — Controlplane Processing & Atomic PostgreSQL CTE/Tx Mutation
- **HTTP Route Registration**: `controlplane/internal/mail/route.go` (`POST /api/v1/tenant/critical/mail/consumers`)
- **HTTP Handler**: `controlplane/internal/mail/transport/http/handler/tenant_consumer_handler.go` (`Create`)
- **Domain Service**: `controlplane/internal/mail/service/tenant_consumer_service_impl.go` (`CreateConsumer`)
- **Taxonomy Errors**: `controlplane/internal/mail/taxonomy/errors.go` (`ErrAlreadyExists`, `ErrTemplateNotFound`, `ErrWorkspaceNotFound`)
- **SQL Repository & CTE/Tx Master**: `controlplane/internal/mail/repository/tenant_consumer_repo_impl.go` (`Create`)
