# Personal Mail Consumer Pause — Master God View

Workflow này là **Source of Truth (SoT) duy nhất** quản lý toàn bộ quy trình tạm dừng (`Pause`) một Mail Consumer cá nhân (Personal Mail Consumer) đang hoạt động trong hệ thống Aurora:
- Tiếp nhận yêu cầu tạm dừng từ giao diện Web Console hoặc SDK qua route trung lập `POST /api/v1/mail/consumers/:id/pause`.
- Xác thực phiên làm việc, kiểm tra bằng chứng phiên (`Session Proof`), bảo vệ chống replay và rewrite URL sang nhánh nội bộ `/api/v1/personal/critical/mail/consumers/:id/pause` tại vùng biên ACR ExtAuthz.
- Xử lý toàn bộ logic nghiệp vụ tại Controlplane: thẩm định quyền hạn RBAC `email:consumer:update`, kiểm tra phiên bản cấu hình (`expected_config_version`), chuyển đổi trạng thái mong muốn sang `PAUSED` (luôn khả dụng, không bị chặn bởi quyền thương mại), niêm phong mã hóa payload Outbox bằng X25519 (`jobpayload.Protector`), và thực thi giao dịch CTE nguyên tử (CTE-First) trên PostgreSQL với **Khóa Thẩm Quyền 4 Chiều**.
...
| **Controlplane Mail Service** | Kiểm tra quyền RBAC `email:consumer:update`, Phiên bản `ConfigVersion`, CTE Tx | PostgreSQL (`mail.personal_mail_consumers`, `mail.mail_outbox_records`) |
...
| `mail.personal_mail_templates` | PostgreSQL (Mail DB) | `SELECT id ... FOR KEY SHARE` | Khóa chia sẻ Template; ngăn chặn việc tạm dừng consumer trỏ vào template đang bị xóa. |
...
| `mail.personal_mail_consumer_update_versions` | PostgreSQL (Mail DB) | `INSERT (desired_state='PAUSED')` | Lưu lịch sử phiên bản cấu hình chuyển sang trạng thái tạm dừng (`PAUSED`). |
...
- **ACR Kiểm tra & Phân quyền Biên**:
  ...
  4. **Path Rewrite**: Chuyển đổi đường dẫn công khai trung lập thành đường dẫn nội bộ nhánh Personal:
     `POST /api/v1/mail/consumers/:id/pause` $\to$ `POST /api/v1/personal/critical/mail/consumers/:id/pause`
...
sequenceDiagram
    ...
    ACR->>ACR: Rewrite path sang /api/v1/personal/critical/mail/consumers/:id/pause
...
sequenceDiagram
    ...
    participant H as PersonalConsumerHandler.Pause
    ...
    Envoy->>Mid: POST /api/v1/personal/critical/mail/consumers/:id/pause
    ...
    Note over DB: CTE thẩm định đồng thời:<br/>- Khóa 4 Chiều (user_id, workspace_id, zone_id, consumer_id)<br/>- Template Tồn tại & Không có Outbox Pending<br/>- Consumer Không có Live Operation Pending<br/>- Insert update_versions (PAUSED) -> Update next_version -> Insert outbox
...
- **Timeline Worker**:
  - Tiêu thụ `OperationSettledEvent` và chèn bản ghi vào bảng `timeline.workspace_events`:
    ...
    INSERT INTO timeline.workspace_events (
        ...
        gen_random_uuid(), $workspace_id, $user_id, 'MAIL_CONSUMER_PAUSED', 'mail_consumer', $consumer_id,
        jsonb_build_object('status', 'PAUSED', 'config_version', 2), NOW()
    );
    ```
- **Audit Logger**:
  - Ghi bản ghi bất biến vào `audit.system_audit_logs`:
    `INSERT INTO audit.system_audit_logs (actor_id, action, resource_type, resource_id, status, created_at) VALUES ($user_id, 'MAIL_CONSUMER_PAUSE', 'mail_consumer', $consumer_id, 'SUCCESS', NOW());`
...
- **Notification Gateway & WebSocket Delivery**:
  ...
  - Đẩy thông điệp realtime qua kết nối WebSocket/SSE đang mở tới Web Console của người dùng để cập nhật Badge `PAUSED` và hiển thị Toast Notification ("Consumer paused successfully").
...
sequenceDiagram
    ...
    par Ghi vết Timeline & Audit
        TimeWorker->>TimeDB: 2a. INSERT INTO timeline.workspace_events (MAIL_CONSUMER_PAUSED)
...
|---|---|---|
| **Người dùng hết tiền / Khóa thanh toán vẫn muốn Pause** | Hệ thống **cho phép** Pause bình thường; không bao giờ khóa quyền Pause của người dùng. | Đảm bảo tính kiểm soát của người dùng đối với luồng tải hạ tầng. |
...
- **HTTP Route Registration**: `controlplane/internal/mail/route.go` (`POST /api/v1/personal/critical/mail/consumers/:id/pause`)
- **HTTP Handler**: `controlplane/internal/mail/transport/http/handler/personal_consumer_handler.go` (`Pause`)
