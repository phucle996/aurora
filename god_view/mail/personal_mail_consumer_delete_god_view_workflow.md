# Personal Mail Consumer Delete & Drain — Master God View

Workflow này là **Source of Truth (SoT) duy nhất** quản lý toàn bộ quy trình xóa và xả cạn (`Delete & Drain`) một Mail Consumer cá nhân (Personal Mail Consumer) trong hệ thống Aurora:
- Tiếp nhận yêu cầu xóa từ giao diện Web Console hoặc SDK qua route trung lập `DELETE /api/v1/mail/consumers/:id`.
...
- Xử lý toàn bộ logic nghiệp vụ tại Controlplane: thẩm định quyền hạn RBAC `email:consumer:delete`, kiểm tra `expected_config_version`, cấp phát hàng rào phiên bản đơn điệu (`Monotonic Tombstone Fence = current.NextConfigVersion`), niêm phong mã hóa payload Outbox bằng X25519 (`jobpayload.Protector`), và thực thi giao dịch CTE nguyên tử (CTE-First) trên PostgreSQL với **Khóa Thẩm Quyền 4 Chiều**.
...
| **Controlplane Mail Service** | Kiểm tra quyền RBAC `email:consumer:delete`, Tombstone Fence, CTE Tx | PostgreSQL (`mail.personal_mail_consumers`, `mail.mail_outbox_records`) |
...
| `mail.personal_mail_consumers` | PostgreSQL (Mail DB) | `SELECT config_version FOR UPDATE` & `DELETE` | Khóa độc quyền và xóa bản ghi consumer; chỉ xóa khi `config_version == expected`. |
...
- **ACR Kiểm tra & Phân quyền Biên**:
  ...
  4. **Path Rewrite**: Chuyển đổi đường dẫn công khai trung lập thành đường dẫn nội bộ nhánh Personal:
     `DELETE /api/v1/mail/consumers/:id` $\to$ `DELETE /api/v1/personal/critical/mail/consumers/:id`
...
| **4. Consumer ID** | Path Param `:id` (UUID) | `mail.personal_mail_consumers.id = $consumer_id` | Khóa độc quyền và xóa đúng dòng Consumer (`FOR UPDATE`). |

### 2. Bảng Ánh Xạ Lỗi Taxonomy (Taxonomy Error Mapping Table)

Khi câu lệnh CTE không xóa được bản ghi (`deleted == false`), tầng Repository phân loại kết quả và ánh xạ chính xác sang lỗi Taxonomy:
...
| `drain_timeout_seconds < 1 \|\| > 300` | `mailTaxonomy.ErrInvalidArgument` | `400 Bad Request` | Thời gian xả cạn `drain_timeout_seconds` phải nằm trong khoảng từ 1 đến 300 giây. |
...
sequenceDiagram
    ...
    participant H as PersonalConsumerHandler.Delete
    ...
    Envoy->>Mid: DELETE /api/v1/personal/critical/mail/consumers/:id
    ...
    Note over S: 5. Cấp phát Tombstone Fence & Chuẩn bị Outbox<br/>tombstoneVersion = current.NextConfigVersion
    ...
    Note over DB: CTE thẩm định đồng thời:<br/>- Khóa 4 Chiều (user_id, workspace_id, zone_id, consumer_id)<br/>- Consumer Không có Live Operation Pending<br/>- DELETE FROM personal_mail_consumers<br/>- INSERT INTO mail_outbox_records (mail.consumer.delete)
    DB-->>Repo: Kết quả CTE (authorized, deleted, outboxID)
    ...
    alt Xung đột phiên bản hoặc có thao tác đang chạy
        ...
        S-->>H: success
        H-->>Envoy: 200 OK ({"message": "consumer deleted successfully"})
...
- **Giải mã X25519**: `CryptoEngine` sử dụng Zone Private Key để giải mã phong ấn, trích xuất Protobuf `MailConsumerDeleteV1` (`ConfigVersion = tombstoneVersion`, `DrainTimeoutSeconds`).
- **Tombstone Fence Enforcement**: So khớp `tombstoneVersion >= current.ConfigVersion`. Đảm bảo Dataplane giải phóng đúng instance cũ và không bị xung đột nếu người dùng tạo lại cùng định danh.
...
- **Xả cạn In-Flight Message trong `DrainTimeoutSeconds`**:
  - Chờ toàn bộ các message đang trong pipeline template rendering & JMAP delivery hoàn tất trong thời gian tối đa `drain_timeout_seconds` (1–300s).
...
- **Chuẩn bị kết quả thực thi**:
  - Đóng gói bản tin `MailJobResultV1` (`job_id = event_id`, `resource_id = consumer_id`, `status = SUCCESS`, `observed_config_version = tombstoneVersion`).
...
- **Dataplane Result Publisher**: Đóng gói Protobuf `MailJobResultV1` với các trường dữ liệu tiêu chuẩn:
  ...
  - `observed_config_version`: Hàng rào tombstone version (`tombstoneVersion`).
...
- **Job Orchestrator Result Listener** tiêu thụ message từ Kafka `aurora.jobs.results.v1`:
  ...
  - **Nhánh `status == FAILED`**:
    ...
    WHERE event_id = $job_id
      AND status IN ('PENDING', 'PROCESSING');
    ```
    - Kích hoạt Deadlock-Buster hoặc Reaper Scheduler để dọn dẹp thủ công.
...
sequenceDiagram
    ...
    DP->>ResKafka: 1. Produce MailJobResultV1 (status = SUCCESS, version = tombstone)
...
- **Timeline Worker**:
  - Tiêu thụ `OperationSettledEvent` và chèn bản ghi vào bảng `timeline.workspace_events`:
    ...
    INSERT INTO timeline.workspace_events (
        ...
        gen_random_uuid(), $workspace_id, $user_id, 'MAIL_CONSUMER_DELETED', 'mail_consumer', $consumer_id,
        jsonb_build_object('status', 'DELETED', 'tombstone_version', $tombstone_version), NOW()
    );
    ```
- **Audit Logger**:
  - Ghi bản ghi bất biến vào `audit.system_audit_logs`:
    `INSERT INTO audit.system_audit_logs (actor_id, action, resource_type, resource_id, status, created_at) VALUES ($user_id, 'MAIL_CONSUMER_DELETE', 'mail_consumer', $consumer_id, 'SUCCESS', NOW());`
...
- **Notification Gateway & WebSocket Delivery**:
  ...
  - Đẩy thông điệp realtime qua kết nối WebSocket/SSE đang mở tới Web Console của người dùng để gỡ bỏ hoàn toàn consumer khỏi bảng danh sách UI.
...
sequenceDiagram
    ...
    par Ghi vết Timeline & Audit
        TimeWorker->>TimeDB: 2a. INSERT INTO timeline.workspace_events (MAIL_CONSUMER_DELETED)
...
### Phase 2 — Controlplane Processing & Atomic PostgreSQL CTE Deletion
- **HTTP Route Registration**: `controlplane/internal/mail/route.go` (`DELETE /api/v1/personal/critical/mail/consumers/:id`)
- **HTTP Handler**: `controlplane/internal/mail/transport/http/handler/personal_consumer_handler.go` (`Delete`)
- **Domain Service**: `controlplane/internal/mail/service/personal_consumer_service_impl.go` (`DeleteConsumer`)
- **Taxonomy Errors**: `controlplane/internal/mail/taxonomy/errors.go` (`ErrVersionConflict`, `ErrOperationInProgress`)
- **SQL Repository & CTE Master**: `controlplane/internal/mail/repository/personal_consumer_repo_impl.go` (`Delete`)
