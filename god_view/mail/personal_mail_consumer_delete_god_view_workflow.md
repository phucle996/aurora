# Personal Mail Consumer Delete — God View
...
This is the personal owner branch, not a self-user API. The browser never selects
an internal owner-prefixed route. Controlplane authorizes `email:consumer:delete`
with the existing required-level selector `"*"`; PostgreSQL rechecks personal
...
`/api/v1/personal/critical/mail/consumers/:id`,
...
`PersonalConsumerHandler.Delete`.
...
The repository locks the scoped consumer and checks `hierarchy.personal_workspaces`: id=workspace, zone_id=Zone, owner_id=actor.
...
[Personal drain workflow](personal_mail_consumer_drain_god_view_workflow.md);
they must be closed before this flow is declared production-ready.

## Phase 5 — JO settles resource-first deletion

Only a valid successful result may remove the personal row, with predicates
...
- `controlplane/internal/mail/transport/http/handler/personal_consumer_handler.go`
...
- `controlplane/internal/mail/domain/entity/personal_consumer.go`
