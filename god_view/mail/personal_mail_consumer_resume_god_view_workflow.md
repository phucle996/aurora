# Personal Mail Consumer Resume & Commercial Admission — Master God View

Workflow này là **Source of Truth (SoT) duy nhất** quản lý toàn bộ quy trình kích hoạt/tiếp tục (`Resume`) một Mail Consumer cá nhân (Personal Mail Consumer) trong hệ thống Aurora:
- Tiếp nhận yêu cầu mở lại luồng tiêu thụ message từ giao diện Web Console hoặc SDK qua route trung lập `POST /api/v1/mail/consumers/:id/resume`.
- Phân giải phiên làm việc, kiểm tra bằng chứng phiên (`Session Proof`), chống tấn công replay và rewrite URL sang nhánh nội bộ `/api/v1/personal/critical/mail/consumers/:id/resume` tại vùng biên ACR ExtAuthz.
- Xử lý toàn bộ logic nghiệp vụ tại Controlplane: thẩm định quyền hạn RBAC `email:consumer:update`, xác thực cấu hình hiện tại, chuẩn hóa Protobuf `MailConsumerUpsertV1` (`DesiredState = ENABLED`), niêm phong mã hóa payload Outbox bằng X25519 (`jobpayload.Protector`), và thực thi giao dịch CTE nguyên tử (CTE-First) trên PostgreSQL với **Khóa Thẩm Quyền 4 Chiều kết hợp Commercial Admission Gate**.
...
| **Controlplane Mail Service** | Kiểm tra quyền RBAC `email:consumer:update`, Admission Check, CTE-First Tx | PostgreSQL (`mail.commercial_admission_projection`, `mail.personal_mail_consumers`, `mail.mail_outbox_records`) |
...
|---|---|---|---|
| `mail.commercial_admission_projection` | PostgreSQL (Mail DB) | `SELECT decision WHERE owner_id=$user_id AND owner_type='PERSONAL'` | Chiếu quyết định thương mại từ Cost; `ALLOW` và `effective_at <= NOW() < valid_until`. |
...
| `mail.personal_mail_templates` | PostgreSQL (Mail DB) | `SELECT id ... FOR KEY SHARE` | Khóa chia sẻ Template; ngăn chặn việc resume consumer trỏ vào template đang bị xóa. |
...
| `mail.personal_mail_consumer_update_versions` | PostgreSQL (Mail DB) | `INSERT (desired_state='ENABLED')` | Lưu lịch sử phiên bản cấu hình mong muốn chuyển sang kích hoạt (`ENABLED`). |
...
- **ACR Kiểm tra & Phân quyền Biên**:
  ...
  4. **Path Rewrite**: Chuyển đổi đường dẫn công khai trung lập thành đường dẫn nội bộ nhánh Personal:
     `POST /api/v1/mail/consumers/:id/resume` $\to$ `POST /api/v1/personal/critical/mail/consumers/:id/resume`
  5. **Header Injection (Trusted Context)**:
     - `x-user-id: <user_uuid>` (Trích xuất từ session đã verify)
     - `x-workspace-id: <workspace_uuid>` (Trích xuất từ ngữ cảnh session/active workspace)
     - `x-zone-id: <zone_uuid>` (Trích xuất từ Zone của workspace)
...
sequenceDiagram
    ...
    ACR->>ACR: Rewrite path sang /api/v1/personal/critical/mail/consumers/:id/resume
...
Toàn bộ quy trình tiếp nhận, thẩm định RBAC, so khớp phiên bản, kiểm tra quyết định thương mại và thực thi giao dịch CTE được xử lý nguyên tử tại Controlplane:
...
| **`DesiredState == 'ENABLED' && !admission_allowed`** | `mailTaxonomy.ErrCommercialAdmissionUnavailable` | **`503 Service Unavailable`** | **Quyền thương mại (`Commercial Admission`) chưa sẵn sàng hoặc đã hết hạn.** |
...
sequenceDiagram
    ...
    participant H as PersonalConsumerHandler.Resume
    ...
    Envoy->>Mid: POST /api/v1/personal/critical/mail/consumers/:id/resume
    ...
    Repo->>DB: Thực thi Master CTE Update (Khóa 4 Chiều + Commercial Gate):
    Note over DB: CTE thẩm định đồng thời:<br/>- Khóa 4 Chiều (user_id, workspace_id, zone_id, consumer_id)<br/>- Commercial Admission == ALLOW & Còn hiệu lực<br/>- Template Tồn tại & Không có Outbox Pending<br/>- Consumer Không có Live Operation Pending<br/>- Insert update_versions -> Update next_version -> Insert outbox
...
- **Timeline Worker**:
  - Tiêu thụ `OperationSettledEvent` và chèn bản ghi vào bảng `timeline.workspace_events`:
    ...
    INSERT INTO timeline.workspace_events (
        ...
        gen_random_uuid(), $workspace_id, $user_id, 'MAIL_CONSUMER_RESUMED', 'mail_consumer', $consumer_id,
        jsonb_build_object('status', 'RUNNING', 'config_version', 2), NOW()
    );
    ```
- **Audit Logger**:
  - Ghi bản ghi bất biến vào `audit.system_audit_logs`:
    `INSERT INTO audit.system_audit_logs (actor_id, action, resource_type, resource_id, status, created_at) VALUES ($user_id, 'MAIL_CONSUMER_RESUME', 'mail_consumer', $consumer_id, 'SUCCESS', NOW());`
...
- **Notification Gateway & WebSocket Delivery**:
  ...
  - Đẩy thông điệp realtime qua kết nối WebSocket/SSE đang mở tới Web Console của người dùng để cập nhật Badge `RUNNING` và hiển thị Toast Notification ("Consumer resumed and actively streaming").
...
sequenceDiagram
    ...
    par Ghi vết Timeline & Audit
        TimeWorker->>TimeDB: 2a. INSERT INTO timeline.workspace_events (MAIL_CONSUMER_RESUMED)
...
|---|---|---|
| **Người dùng chưa nạp tiền hoặc bị khóa thanh toán** | CTE kiểm tra `commercial_admission_projection` thấy `ALLOW` không tồn tại hoặc `decision != 'ALLOW'` $\to$ Không chèn bản ghi version và outbox, trả về `503 Service Unavailable` (`ErrCommercialAdmissionUnavailable`). | Người dùng hoàn tất nạp tiền vào ví cá nhân $\to$ Cost Manager phát event cập nhật projection sang `ALLOW` $\to$ Cho phép Resume lại. |
...
| **Người dùng bấm Resume đồng thời ở 2 thiết bị (Concurrent Race)** | `target` CTE sử dụng `FOR UPDATE` trên dòng consumer. Request đầu tiên tăng `next_config_version`; request thứ hai thấy `config_version` không khớp $\to$ Trả về `409 Conflict` (`ErrVersionConflict`). | UI nhận lỗi 409 và làm mới trạng thái hiển thị phiên bản mới nhất. |
...
| **Payload Outbox bị giả mạo trên đường truyền** | Payload đã được mã hóa niêm phong X25519 (`jobpayload.Protector`). Dataplane không giải mã được chữ ký xác thực $\to$ Đẩy lỗi về kết quả và không khởi chạy worker. | Bảo đảm tính toàn vẹn tuyệt đối của dữ liệu cấu hình nhạy cảm (Stream Credentials). |
...
### Phase 2 — Controlplane Processing & Atomic PostgreSQL CTE Mutation
- **HTTP Route Registration**: `controlplane/internal/mail/route.go` (`POST /api/v1/personal/critical/mail/consumers/:id/resume`)
- **HTTP Handler**: `controlplane/internal/mail/transport/http/handler/personal_consumer_handler.go` (`Resume`)
- **Domain Service**: `controlplane/internal/mail/service/personal_consumer_service_impl.go` (`ChangeConsumerState`, `UpdateConsumer`)
- **Taxonomy Errors**: `controlplane/internal/mail/taxonomy/errors.go` (`ErrCommercialAdmissionUnavailable`, `ErrVersionConflict`)
