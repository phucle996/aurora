# Tenant Mail Consumer Update — Master God View

Workflow này là **Source of Truth (SoT) duy nhất** quản lý toàn bộ quy trình cập nhật cấu hình của một Mail Consumer doanh nghiệp/tổ chức (Tenant Mail Consumer) trong hệ thống Aurora:
- Tiếp nhận yêu cầu cập nhật từ giao diện Web Console của Tenant hoặc Backend Server SDK qua route trung lập `PATCH /api/v1/mail/consumers/:id`.
...
- Xử lý toàn bộ logic nghiệp vụ tại Controlplane: thẩm định quyền hạn RBAC `email:consumer:update` trên phạm vi Tenant Workspace, bảo vệ an toàn danh tính AAD (`SourceConfigEnvelope`), cấp phát `NextConfigVersion`, tính mã băm `ConfigSHA256`, niêm phong mã hóa payload Outbox bằng X25519 (`jobpayload.Protector`), và thực thi giao dịch CTE nguyên tử (CTE-First) trên PostgreSQL với **Khóa Thẩm Quyền 4 Chiều kết hợp Ràng buộc Tenant Membership**.
...
| **Controlplane Mail Service** | Kiểm tra quyền RBAC `email:consumer:update`, AAD Guard, CTE-First Tx | PostgreSQL (`mail.tenant_mail_consumers`, `mail.mail_outbox_records`) |
...
| `mail.tenant_mail_templates` | PostgreSQL (Mail DB) | `SELECT id ... FOR KEY SHARE` | Khóa chia sẻ Template; ngăn chặn việc cập nhật consumer trỏ vào template đang bị xóa. |
...
| `mail.tenant_mail_consumer_update_versions` | PostgreSQL (Mail DB) | `INSERT (version = current_version + 1)` | Lưu lịch sử phiên bản cấu hình đã cập nhật của Tenant Consumer. |
...
- **JSON Request Body**:
  ```json
  {
    ...
    "name": "Updated Tenant Orders Consumer",
    "source_type": "kafka",
    "broker_resource_id": "0194f83a-8b1e-7d34-92c1-382a1d820050",
    "source_config_envelope": "",
    "topic": "tenant.orders.created.v2",
    "consumer_group": "tenant-mail-order-notifier",
    "template_id": "tenant-order-confirmation-tpl",
    "template_version": 2,
    "sender_profile_id": "tenant-billing-noreply",
    "sender_version": 1,
...
- **ACR Kiểm tra & Phân quyền Biên**:
  ...
  5. **Path Rewrite**: Chuyển đổi đường dẫn công khai trung lập thành đường dẫn nội bộ nhánh Tenant:
     `PATCH /api/v1/mail/consumers/:id` $\to$ `PATCH /api/v1/tenant/critical/mail/consumers/:id`
...
|---|---|---|---|
| **1. Tenant ID & Membership** | Trusted Header `x-tenant-id` & `x-user-id` | `hierarchy.tenant_memberships.tenant_id = $tenant_id AND user_id = $actor_id AND status = 'active'` | Ngăn chặn việc cập nhật consumer khi user không còn là thành viên active của Tenant. |
...
sequenceDiagram
    ...
    participant H as TenantConsumerHandler.Update
    ...
    Envoy->>Mid: PATCH /api/v1/tenant/critical/mail/consumers/:id
    ...
    alt AAD thay đổi nhưng thiếu Envelope mới
        ...
        Note over DB: CTE thẩm định đồng thời:<br/>- Khóa 4 Chiều (tenant_id, membership, workspace_id, zone_id, consumer_id)<br/>- Template Tồn tại & Không có Outbox Pending<br/>- Consumer Không có Live Operation Pending<br/>- Insert update_versions -> Update next_version -> Insert outbox
...
- **Timeline Worker**:
  - Tiêu thụ `OperationSettledEvent` và chèn bản ghi vào bảng `timeline.workspace_events`:
    ...
    INSERT INTO timeline.workspace_events (
        ...
        gen_random_uuid(), $workspace_id, $actor_user_id, 'MAIL_CONSUMER_UPDATED', 'mail_consumer', $consumer_id,
        jsonb_build_object('config_version', 3, 'tenant_id', $tenant_id), NOW()
    );
    ```
- **Audit Logger**:
  - Ghi bản ghi bất biến vào `audit.system_audit_logs` phục vụ tra cứu tuân thủ doanh nghiệp:
    `INSERT INTO audit.system_audit_logs (actor_id, tenant_id, action, resource_type, resource_id, status, created_at) VALUES ($actor_user_id, $tenant_id, 'MAIL_CONSUMER_UPDATE', 'mail_consumer', $consumer_id, 'SUCCESS', NOW());`
...
- **Notification Gateway & WebSocket Delivery**:
  ...
  - Đẩy thông điệp realtime qua kết nối WebSocket/SSE đang mở tới Web Console của Tenant để cập nhật dữ liệu cấu hình mới nhất và hiển thị Toast Notification.
...
sequenceDiagram
    ...
    par Ghi vết Timeline & Audit
        TimeWorker->>TimeDB: 2a. INSERT INTO timeline.workspace_events (MAIL_CONSUMER_UPDATED, tenant_id)
...
| **User bị gỡ khỏi Tenant trước khi request tới DB** | CTE kiểm tra `tenant_memberships` không tìm thấy bản ghi `active` $\to$ Trả về `404 Not Found` (`ErrConsumerNotFound`). | Ngăn chặn hành vi chỉnh sửa cấu hình trái phép sau khi rời tổ chức. |
...
### Phase 2 — Controlplane Processing & Atomic PostgreSQL CTE Mutation
- **HTTP Route Registration**: `controlplane/internal/mail/route.go` (`PATCH /api/v1/tenant/critical/mail/consumers/:id`)
- **HTTP Handler**: `controlplane/internal/mail/transport/http/handler/tenant_consumer_handler.go` (`Update`)
- **Domain Service**: `controlplane/internal/mail/service/tenant_consumer_service_impl.go` (`UpdateConsumer`)
