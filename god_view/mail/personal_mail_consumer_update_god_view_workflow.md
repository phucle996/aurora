# Personal Mail Consumer Update — Master God View

Workflow này là **Source of Truth (SoT) duy nhất** quản lý toàn bộ quy trình cập nhật cấu hình của một Mail Consumer cá nhân (Personal Mail Consumer) trong hệ thống Aurora:
- Tiếp nhận yêu cầu cập nhật từ giao diện Web Console hoặc SDK qua route trung lập `PATCH /api/v1/mail/consumers/:id`.
- Xác thực phiên làm việc, kiểm tra bằng chứng phiên (`Session Proof`), bảo vệ chống replay và rewrite URL sang nhánh nội bộ `/api/v1/personal/critical/mail/consumers/:id` tại vùng biên ACR ExtAuthz.
- Xử lý toàn bộ logic nghiệp vụ tại Controlplane: thẩm định quyền hạn RBAC `email:consumer:update`, bảo vệ an toàn danh tính AAD (`SourceConfigEnvelope`), cấp phát `NextConfigVersion`, tính mã băm `ConfigSHA256`, niêm phong mã hóa payload Outbox bằng X25519 (`jobpayload.Protector`), và thực thi giao dịch CTE nguyên tử (CTE-First) trên PostgreSQL với **Khóa Thẩm Quyền 4 Chiều**.
...
- Dataplane tại Zone tiêu thụ lệnh từ Kafka, giải mã mật mã X25519 bằng Zone Private Key, thực hiện Hot-Reload dynamic subscription (Topic, Consumer Group, Parallelism, Template Version) mà không làm rớt message.
- Dataplane đóng gói bản tin kết quả Protobuf `MailJobResultV1`, đẩy sang Kafka Result Topic; Job Orchestrator Result Listener tiêu thụ và thực hiện tất toán nguyên tử (`status = 'SETTLED'`) trên PostgreSQL, sau đó kích hoạt chuyển giao sang Phase 6.
...
| **Controlplane Mail Service** | Kiểm tra quyền RBAC `email:consumer:update`, AAD Guard, CTE-First Tx | PostgreSQL (`mail.personal_mail_consumers`, `mail.mail_outbox_records`) |
...
| **Dataplane (Zone Worker)** | Hot-reload Consumer Group, cập nhật Template Binding tại Zone | Zone In-Memory Consumer Instance |
...
| `mail.personal_mail_templates` | PostgreSQL (Mail DB) | `SELECT id ... FOR KEY SHARE` | Khóa chia sẻ Template; ngăn chặn việc cập nhật consumer trỏ vào template đang bị xóa. |
| `mail.personal_mail_consumers` | PostgreSQL (Mail DB) | `SELECT config_version FOR UPDATE` & `UPDATE next_config_version` | Khóa độc quyền bản ghi consumer để ngăn ngừa xung đột phiên bản (Optimistic Concurrency). |
| `mail.personal_mail_consumer_update_versions` | PostgreSQL (Mail DB) | `INSERT (version = current_version + 1)` | Lưu lịch sử phiên bản cấu hình đã cập nhật. |
...
- **Method & Path**: `PATCH /api/v1/mail/consumers/0194f83a-8b1e-7d34-92c1-382a1d820001`
- **Headers**:
...
- **ACR Kiểm tra & Phân quyền Biên**:
  ...
  4. **Path Rewrite**: Chuyển đổi đường dẫn công khai trung lập thành đường dẫn nội bộ nhánh Personal:
     `PATCH /api/v1/mail/consumers/:id` $\to$ `PATCH /api/v1/personal/critical/mail/consumers/:id`
...
sequenceDiagram
    ...
    Client->>Envoy: 1. PATCH /api/v1/mail/consumers/:id (JSON Payload)
    ...
    ACR->>ACR: Rewrite path sang /api/v1/personal/critical/mail/consumers/:id
...
Toàn bộ quy trình xác thực quyền hạn, kiểm tra AAD identity guard, niêm phong mật mã X25519 và thực thi giao dịch CTE trên cơ sở dữ liệu được xử lý nguyên tử tại Controlplane:
...
| **3. Zone ID** | Trusted Header `x-zone-id` | `hierarchy.personal_workspaces.zone_id = $zone_id` | Bảo đảm tài nguyên thuộc đúng Zone của Workspace. |
| **4. Consumer ID** | Path Param `:id` (UUID) | `mail.personal_mail_consumers.id = $consumer_id` | Khóa độc quyền đúng dòng Consumer (`FOR UPDATE`). |
...
|---|---|---|---|
| `!authorized \|\| currentVersion == 0` | `mailTaxonomy.ErrConsumerNotFound` | `404 Not Found` | Consumer không tồn tại hoặc không thuộc quyền sở hữu của User trong Workspace. |
...
| Thay đổi AAD identity nhưng thiếu Envelope | `mailTaxonomy.ErrInvalidArgument` | `400 Bad Request` | Đổi Broker hoặc SourceType nhưng không truyền lại `source_config_envelope` mới. |
...
sequenceDiagram
    ...
    participant H as PersonalConsumerHandler.Update
    ...
    Envoy->>Mid: PATCH /api/v1/personal/critical/mail/consumers/:id
    ...
    H->>H: 2. Parse UUID và bind UpdateConsumerRequest
    H->>S: UpdateConsumer(ctx, command)
    ...
    Note over S: 4. AAD Preservation Check & Chuẩn bị Outbox
    alt AAD thay đổi nhưng thiếu Envelope mới
        S-->>H: ErrInvalidArgument (400 Bad Request)
        H-->>Envoy: 400 Bad Request
    else Hợp lệ
        S->>Repo: Update(ctx, consumer, outbox)
        Repo->>Prot: 5. Seal(ctx, metadata, outbox.Payload)
        Prot-->>Repo: Payload đã mã hóa X25519 & KeyID

        Note over Repo,DB: 6. Mở Transaction PostgreSQL (tx.BeginTx)
        Repo->>DB: Thực thi Master CTE Update (Khóa 4 Chiều)
        Note over DB: CTE thẩm định đồng thời:<br/>- Khóa 4 Chiều (user_id, workspace_id, zone_id, consumer_id)<br/>- Template Tồn tại & Không có Outbox Pending<br/>- Consumer Không có Live Operation Pending<br/>- Insert update_versions -> Update next_version -> Insert outbox
        ...
        alt Xung đột phiên bản hoặc có thao tác đang chạy
            ...
            Repo->>DB: 7. COMMIT Transaction
            Repo-->>S: nil (Thành công)
            S-->>H: updated consumer entity
            H-->>Envoy: 200 OK (Updated Consumer Entity)
...
## ⚙️ Phase 4 — Dataplane Stream Execution & Consumer Hot Reload

Phase 4 chịu trách nhiệm tiếp nhận bản tin cập nhật, giải mã X25519 và thực hiện **Hot Reload Dynamic Subscription** mà không làm gián đoạn luồng stream:
...
- **Giải mã X25519**: `CryptoEngine` sử dụng Zone Private Key để giải mã phong ấn, trích xuất Protobuf `MailConsumerUpsertV1` chứa cấu hình mới (`Parallelism`, `Topic`, `ConsumerGroup`, `TemplateVersion`).
- **Monotonic Version Gate**: So khớp `ConfigVersion == current.ConfigVersion + 1`. Nếu version không tuần tự (out-of-order), Dataplane nạp lại snapshot từ Controlplane.

### 2. Quản lý Vòng đời Adapter & Hot-Reload Cấu hình (Dynamic Rebalance & Cache Update)
- **Hot-Reload Broker Client**:
  - Nếu `Topic`, `ConsumerGroup` hoặc `Parallelism` thay đổi: `BrokerAdapter` thực hiện dynamic partition rebalance hoặc điều chỉnh số lượng worker goroutines mà không ngắt TCP connection chính.
  - Nếu `TemplateVersion` hoặc `SenderVersion` thay đổi: `ConsumerRegistry` cập nhật con trỏ template binding trong bộ nhớ, các message tiếp theo sẽ tự động được render theo phiên bản template mới.
...
sequenceDiagram
    ...
    Kafka-->>Ingress: 1. Tiêu thụ Message MailConsumerUpsertV1 (candidate version)
    Ingress->>Crypto: 2. Yêu cầu giải mã sealed payload bằng Zone Private Key
    Crypto-->>Ingress: 3. Payload Protobuf đã giải mã (New Topic, Parallelism, TemplateVersion)

    Ingress->>Reg: 4. Cập nhật Registry (ConfigVersion = candidate, TemplateVersion = N)
    Reg-->>Ingress: OK

    Ingress->>Adapter: 5. Yêu cầu Hot-Reload Subscription & Worker Pool
    Adapter->>Broker: 6. Dynamic Rebalance / Topic Resubscribe
    Broker-->>Adapter: 7. Resubscribed successfully (No message loss)
...
Phase 5 chịu trách nhiệm hoàn tất vòng đời của tác vụ phân tán: Dataplane báo cáo kết quả hot-reload cấu hình thành công về kênh kết quả tập trung, và Job Orchestrator thực hiện tất toán nguyên tử trên Outbox table:
...
- **Dataplane Result Publisher**: Đóng gói Protobuf `MailJobResultV1` với các trường dữ liệu tiêu chuẩn:
  ...
  - `status`: `JOB_STATUS_SUCCESS` (hoặc `JOB_STATUS_FAILED` nếu rebalance thất bại).
  - `observed_config_version`: Phiên bản cấu hình mới đã hot-reload thành công (`expected_config_version + 1`).
...
- **Job Orchestrator Result Listener** tiêu thụ message từ Kafka `aurora.jobs.results.v1`:
  - **Nhánh `status == SUCCESS`**:
    ...
    ```
    - Xác nhận phiên bản cấu hình `config_version = N+1` đã áp dụng toàn diện trên Dataplane.
  ...
  - **Nhánh `status == FAILED`**:
    ...
    WHERE event_id = $job_id
      AND status IN ('PENDING', 'PROCESSING');
    ```
    - Chuyển `operational_status` sang `DEGRADED` (Config Mismatch).
...
sequenceDiagram
    ...
    DP->>ResKafka: 1. Produce MailJobResultV1 (status = SUCCESS, version = N+1)
...
- **Timeline Worker**:
  - Tiêu thụ `OperationSettledEvent` và chèn bản ghi vào bảng `timeline.workspace_events`:
    ...
    INSERT INTO timeline.workspace_events (
        ...
        gen_random_uuid(), $workspace_id, $user_id, 'MAIL_CONSUMER_UPDATED', 'mail_consumer', $consumer_id,
        jsonb_build_object('config_version', 3), NOW()
    );
    ```
- **Audit Logger**:
  - Ghi bản ghi bất biến vào `audit.system_audit_logs`:
    `INSERT INTO audit.system_audit_logs (actor_id, action, resource_type, resource_id, status, created_at) VALUES ($user_id, 'MAIL_CONSUMER_UPDATE', 'mail_consumer', $consumer_id, 'SUCCESS', NOW());`
...
- **Phát Event sang Notification Bus**:
  - JO đẩy bản tin thông báo vào topic `aurora.notifications.events.v1`:
    ```json
    {
      "event_type": "MAIL_CONSUMER_UPDATED",
...
- **Notification Gateway & WebSocket Delivery**:
  ...
  - Đẩy thông điệp realtime qua kết nối WebSocket/SSE đang mở tới Web Console của người dùng để cập nhật dữ liệu cấu hình mới nhất và hiển thị Toast Notification.
...
sequenceDiagram
    ...
    par Ghi vết Timeline & Audit
        TimeWorker->>TimeDB: 2a. INSERT INTO timeline.workspace_events (MAIL_CONSUMER_UPDATED)
        TimeDB-->>TimeWorker: Saved
    and
        Bus-->>AuditLog: 2b. Tiêu thụ Audit Event
        AuditLog->>AuditDB: 2c. INSERT INTO audit.system_audit_logs (MAIL_CONSUMER_UPDATE, SUCCESS)
    ...
    and
        ...
        NotiGW->>Client: 4. WebSocket Push Event (Updated v3, Toast Noti)
    end
```
...
|---|---|---|
| **Thay đổi Broker/SourceType nhưng không gửi lại Envelope** | Service phát hiện vi phạm bảo vệ danh tính AAD $\to$ Trả về ngay `400 Bad Request ("invalid request")`. | Client phải cung cấp `source_config_envelope` mới tương ứng với Broker mới. |
| **Xung đột phiên bản khi nhiều người cùng chỉnh sửa (Optimistic Concurrency)** | `target` CTE kiểm tra `config_version != expected_config_version` $\to$ Trả về `409 Conflict` (`ErrVersionConflict`). | UI nhận mã lỗi 409, fetch lại phiên bản mới nhất từ DB và cho phép người dùng merge thay đổi. |
| **Consumer đang có một Outbox Operation chưa hoàn tất** | `live_operation` CTE phát hiện bản ghi outbox của consumer ở trạng thái `PENDING`/`PROCESSING` $\to$ Trả về `409 Conflict` (`ErrOperationInProgress`). | Người dùng đợi vài giây cho outbox job trước đó settled rồi thử lại. |
...
### Phase 2 — Controlplane Processing & Atomic PostgreSQL CTE Mutation
- **HTTP Route Registration**: `controlplane/internal/mail/route.go` (`PATCH /api/v1/personal/critical/mail/consumers/:id`)
- **HTTP Handler**: `controlplane/internal/mail/transport/http/handler/personal_consumer_handler.go` (`Update`)
- **Domain Service**: `controlplane/internal/mail/service/personal_consumer_service_impl.go` (`UpdateConsumer`)
- **Taxonomy Errors**: `controlplane/internal/mail/taxonomy/errors.go` (`ErrVersionConflict`, `ErrOperationInProgress`)
- **SQL Repository & CTE Master**: `controlplane/internal/mail/repository/personal_consumer_repo_impl.go` (`Update`)
...
### Phase 4 — Dataplane Stream Execution & Consumer Hot Reload
