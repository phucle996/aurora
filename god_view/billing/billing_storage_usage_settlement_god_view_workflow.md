...
**Implementation status:** the Protobuf contract, JO Kafka-to-Shared-Redis relay,
and Cost Engine report consumer are implemented behind the explicit
`STORAGE_REPORT_SETTLEMENT_ENABLED` flag. The flag defaults to `false`, so the
legacy Central ClickHouse path remains the only charge-producing path until the
cutover gates are approved. Both charge-producing paths must never run together.
...
The opt-in Cost Engine consumer reads the Redis stream with the named consumer
group and reclaim timeout. It validates the stream payload again because Redis
is a transport boundary, checks stream identity fields against the canonical
payload, and creates a durable report inbox plus deterministic line identities.
It resolves the resource owner in Billing PostgreSQL using the report window and
the approved ownership-generation rule. Missing or ambiguous ownership becomes
durable `unrated_usage`; the engine never guesses a wallet from a client
credential or resource name.
...
sequenceDiagram
    ...
    E->>I: Insert or lock report and line inbox identities
    E->>B: Resolve resource owner at the report time rule
    alt missing or ambiguous owner
        E->>I: Insert durable unrated usage and mark line UNRATED
        E->>R: XACK/XDEL after PostgreSQL commit
    else owner resolved
        E->>I: Keep the line PENDING for the settlement transaction
    end
```

## Phase 3 — Atomic wallet and ledger settlement

For a resolved line, the worker obtains the billing-run pricing snapshot, locks
exactly one wallet, and writes report/line inbox state, wallet mutation and
immutable ledger entry in one PostgreSQL transaction. The ledger identity is
deterministic from report ID, aggregate resource and direction. A pre-existing
ledger identity reconciles the line without a second debit. Redis is acknowledged
and deleted only after commit and an intact billing fence; an ACK/XDEL error
...
sequenceDiagram
    ...
    P->>P: Lock report/line inbox identity and use pinned price snapshot
    P->>W: Lock wallet row
    W-->>P: Current balance
    P->>W: Apply deterministic debit or create unrated state
    P->>L: Insert immutable ledger identity
    P->>P: Mark line and report SETTLED or UNRATED
    P-->>E: Commit
    E->>R: XACK then XDEL only after commit and intact fence
```
...
no-op, not a second debit. A correction is accepted by the wire validator but is
currently persisted as `DEAD` with
`STORAGE_USAGE_CORRECTION_POLICY_NOT_ENABLED` until the signed adjustment policy
is approved. No settled history is edited or deleted, and no wallet is mutated
for a correction in this rollout.
...
| Corrected report | Durable DEAD quarantine until adjustment policy is enabled; no debit |
...
- Opt-in owner: `cost-manager/engine/src/service/storage/usage_report_settlement.rs`
