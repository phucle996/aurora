# Resource Lifecycle Event Pipeline — God View

> [!IMPORTANT]
> Tài liệu này là **Source of Truth (SoT) duy nhất** cho toàn bộ pipeline lifecycle event từ bucket create/delete trong Controlplane cho đến ownership projection trong Billing.
...
3. [Contract Protobuf ResourceLifecycleEventV1](#3-contract-protobuf-resourcelifecycleeventv1)
...
- Mọi bucket create/delete thành công đều phát **durable lifecycle event** vào JetStream.
...
## 3. Contract Protobuf ResourceLifecycleEventV1
...
message ResourceLifecycleEventV1 {
  ...
  string effective_at    = 11; // RFC3339 UTC — thời điểm lifecycle event có hiệu lực
...
- `SUCCEEDED`: UPDATE outbox, set `completed_at`. Đồng thời trong cùng TX: insert lifecycle event.
...
sequenceDiagram
    ...
    JS-->>CM: ResourceLifecycleEventV1
    CM->>BDB: BEGIN TX
    ...
    CM->>BDB: UPSERT resource_lifecycle_head (source_version)
    CM->>BDB: COMMIT TX
    CM->>JS: ACK
```
...
Xử lý DELETE:
  ...
  → UPSERT resource_lifecycle_head: {resource_id, last_source_version=2, lifecycle_state=DELETED}
...
Sau đó RESOURCE_CREATED (source_version=1) đến:
  ...
  → Đọc resource_lifecycle_head: lifecycle_state=DELETED, last_source_version=2
...
| Tình huống | Xử lý |
|---|---|
| Relay crash sau INSERT lifecycle event nhưng trước PubAck | Relay polling `UNPUBLISHED` retry; `Nats-Msg-Id=event_id` JetStream dedup |
...
- Subject filter: `controlplane.storage.resource.lifecycle.v1`
- Storage: `File`
- Retention: `Limits`
...
| Billing inbox schema | `cost-manager/api/migrations/000002_tables.up.sql` (billing.ownership_event_inbox & resource_lifecycle_head) |
