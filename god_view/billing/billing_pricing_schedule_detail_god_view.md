...
This workflow returns one controlled schedule and its latest immutable version.
It is not a quote and does not select a wallet or promise a monthly amount.
...
sequenceDiagram
    ...
    R->>DB: lock-free schedule read + latest version lateral selection
    ...
    S-->>H: detail
    H-->>CI: 200 JSON schedule + latest version/brackets
```
