...
func NewMailPricingRepository(db *pgxpool.Pool) billingRepoInterface.MailPricingRepository {
	return &mailPricingRepository{db: db}
}

func (r *mailPricingRepository) GetActiveMailPricingSnapshot(ctx context.Context, chargeKind entity.ChargeKindCode, at time.Time) (*entity.MailPricingSnapshot, error) {
	rows, err := r.db.Query(ctx, `WITH winner AS (
		SELECT s.id AS schedule_id,s.code,s.charge_kind_code,c.module_code,c.raw_input_unit,s.pricing_model,s.currency,v.id AS version_id,v.version_number,v.effective_from,v.effective_to,v.checksum
		FROM billing.pricing_schedules s JOIN billing.charge_kind_catalog c ON c.code=s.charge_kind_code JOIN billing.pricing_schedule_versions v ON v.pricing_schedule_id=s.id
		WHERE s.charge_kind_code=$1 AND c.module_code='mail' AND s.status='ACTIVE' AND v.status <> 'CANCELLED' AND v.effective_from <= $2 AND (v.effective_to IS NULL OR $2 < v.effective_to)
		ORDER BY v.effective_from DESC,s.id LIMIT 1
	) SELECT w.schedule_id,w.code,w.charge_kind_code,w.module_code,w.raw_input_unit,w.pricing_model,w.currency,w.version_id,w.version_number,w.effective_from,w.effective_to,w.checksum,b.id,b.range_start_quantity,b.range_end_quantity,b.price_numerator_micro_units,b.price_denominator_quantity FROM winner w JOIN billing.pricing_schedule_scalar_brackets b ON b.pricing_schedule_version_id=w.version_id ORDER BY b.range_start_quantity`, string(chargeKind), at)
	if err != nil {
		return nil, fmt.Errorf("Mail pricing repo: active base snapshot: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&scheduleID, &code, &kind, &module, &unit, &model, &currency, &versionID, &version, &effectiveFrom, &effectiveTo, &checksum, &bracketID, &start, &end, &numerator, &denominator); err != nil {
			return nil, fmt.Errorf("Mail pricing repo: scan active base snapshot: %w", err)
		}
		if snapshot == nil {
			snapshot = &entity.MailPricingSnapshot{PricingScheduleID: scheduleID, VersionID: versionID, Code: code, ChargeKindCode: entity.ChargeKindCode(kind), ModuleCode: module, RawInputUnit: unit, PricingModel: entity.PricingModel(model), Currency: currency, VersionNumber: version, EffectiveFrom: effectiveFrom, EffectiveTo: effectiveTo, Checksum: checksum}
		}
		snapshot.Brackets = append(snapshot.Brackets, entity.MailPricingSnapshotBracket{ID: bracketID, RangeStartQuantity: start, RangeEndQuantity: end, PriceNumeratorMicroUnits: numerator, PriceDenominatorQuantity: denominator})
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("Mail pricing repo: iterate active base snapshot: %w", err)
	}
	if snapshot == nil {
...
func (r *mailPricingRepository) GetMailBasePricePublishTarget(ctx context.Context, code string) (*entity.MailBasePricePublishTarget, error) {
	var target entity.MailBasePricePublishTarget
	var kind, model string
	err := r.db.QueryRow(ctx, `WITH target AS (SELECT s.id,s.code,s.charge_kind_code,s.pricing_model,s.currency FROM billing.pricing_schedules s JOIN billing.charge_kind_catalog k ON k.code=s.charge_kind_code WHERE s.code=$1 AND s.status='ACTIVE' AND k.module_code='mail' AND s.charge_kind_code='mail.delivery.accepted_recipient') SELECT id,code,charge_kind_code,pricing_model,currency FROM target`, code).Scan(&target.PricingScheduleID, &target.ScheduleCode, &kind, &model, &target.Currency)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, billingTaxonomy.ErrPricingScheduleNotFound
	}
	if err != nil {
		return nil, fmt.Errorf("Mail pricing repo: base publish target: %w", err)
	}
...
func (r *mailPricingRepository) CreateMailBasePriceVersion(ctx context.Context, create entity.MailBasePricePublishCommand, brackets []entity.MailBasePriceBracketCommand) (*entity.MailBasePricePublished, error) {
	tx, err := r.db.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return nil, fmt.Errorf("Mail pricing repo: begin base publish: %w", err)
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	var scheduleID uuid.UUID
	var kind, model string
	err = tx.QueryRow(ctx, `WITH target AS (SELECT s.id,s.charge_kind_code,s.pricing_model::text FROM billing.pricing_schedules s JOIN billing.charge_kind_catalog k ON k.code=s.charge_kind_code WHERE s.code=$1 AND s.status='ACTIVE' AND k.module_code='mail' AND s.charge_kind_code='mail.delivery.accepted_recipient' FOR UPDATE OF s) SELECT id,charge_kind_code,pricing_model FROM target`, create.ScheduleCode).Scan(&scheduleID, &kind, &model)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, billingTaxonomy.ErrPricingScheduleNotFound
	}
	if err != nil {
		return nil, fmt.Errorf("Mail pricing repo: lock base schedule: %w", err)
	}
	var latest int
	...
	if errors.Is(err, pgx.ErrNoRows) {
		...
		return nil, fmt.Errorf("Mail pricing repo: latest base version: %w", err)
	}
	...
	if latest > 0 {
		if _, err = tx.Exec(ctx, `UPDATE billing.pricing_schedule_versions SET effective_to=$1,status=CASE WHEN status='ACTIVE' THEN 'SUPERSEDED' ELSE status END WHERE pricing_schedule_id=$2 AND version_number=$3 AND effective_to IS NULL`, create.EffectiveFrom, scheduleID, latest); err != nil {
			return nil, fmt.Errorf("Mail pricing repo: close prior base version: %w", err)
		}
	}
	if _, err = tx.Exec(ctx, `INSERT INTO billing.pricing_schedule_versions (id,pricing_schedule_id,pricing_model,version_number,status,effective_from,checksum,change_reason,created_by) VALUES ($1,$2,$3::billing.pricing_model,$4,$5,$6,$7,$8,$9)`, versionID, scheduleID, model, latest+1, status, create.EffectiveFrom, create.Checksum, create.ChangeReason, create.CreatedBy); err != nil {
		return nil, fmt.Errorf("Mail pricing repo: insert base version: %w", err)
	}
	for _, bracket := range brackets {
		if _, err = tx.Exec(ctx, `INSERT INTO billing.pricing_schedule_scalar_brackets (id,pricing_schedule_version_id,range_start_quantity,range_end_quantity,price_numerator_micro_units,price_denominator_quantity) VALUES ($1,$2,$3,$4,$5,$6)`, uuid.New(), versionID, bracket.RangeStartQuantity, bracket.RangeEndQuantity, bracket.PriceNumeratorMicroUnits, bracket.PriceDenominatorQuantity); err != nil {
			return nil, fmt.Errorf("Mail pricing repo: insert base bracket: %w", err)
		}
	}
	if _, err = tx.Exec(ctx, `INSERT INTO billing.pricing_outbox (id,event_type,pricing_schedule_id,version_id,module_code,charge_kind_code,effective_from,checksum) VALUES ($1,'PRICING_SCHEDULE_VERSION_PUBLISHED',$2,$3,'mail',$4,$5,$6)`, uuid.New(), scheduleID, versionID, kind, create.EffectiveFrom, create.Checksum); err != nil {
		return nil, fmt.Errorf("Mail pricing repo: insert base outbox: %w", err)
	}
	if err = tx.Commit(ctx); err != nil {
		return nil, fmt.Errorf("Mail pricing repo: commit base publish: %w", err)
	}
	return &entity.MailBasePricePublished{ID: versionID, PricingScheduleID: scheduleID, ChargeKindCode: entity.ChargeKindCode(kind), VersionNumber: latest + 1, PricingModel: entity.PricingModel(model), Status: status, EffectiveFrom: create.EffectiveFrom, Checksum: create.Checksum}, nil
}

func (r *mailPricingRepository) ListMailZonePriceAdjustments(ctx context.Context, query entity.MailZoneAdjustmentListQuery) ([]entity.MailZoneAdjustmentListItem, bool, error) {
	rows, err := r.db.Query(ctx, `
		WITH history AS (
			...
			FROM billing.mail_zone_price_adjustment_versions WHERE zone_id=$1
	...
	if err != nil {
		return nil, false, fmt.Errorf("Mail pricing repo: list Zone adjustments: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.ZoneID, &item.VersionNumber, &item.Status, &item.EffectiveFrom, &item.EffectiveTo, &item.MultiplierNumerator, &item.MultiplierDenominator, &item.Checksum, &item.ChangeReason, &item.CreatedBy, &item.CreatedAt, &item.IsLatest, &item.IsEffective); err != nil {
			return nil, false, fmt.Errorf("Mail pricing repo: scan Zone adjustment: %w", err)
		}
		items = append(items, item)
	}
	if err := rows.Err(); err != nil {
		return nil, false, fmt.Errorf("Mail pricing repo: iterate Zone adjustments: %w", err)
	}
...
func (r *mailPricingRepository) RefreshMailPricingStatuses(ctx context.Context) error {
	if _, err := r.db.Exec(ctx, `WITH projected AS (
		...
		WHERE v.status<>'CANCELLED' AND k.module_code='mail')
		...
		return fmt.Errorf("Mail pricing repo: refresh base statuses: %w", err)
	}
	if _, err := r.db.Exec(ctx, `WITH projected AS (
		...
		FROM billing.mail_zone_price_adjustment_versions WHERE status<>'CANCELLED')
		UPDATE billing.mail_zone_price_adjustment_versions v SET status=projected.desired_status FROM projected WHERE v.id=projected.id AND v.status IS DISTINCT FROM projected.desired_status`); err != nil {
		return fmt.Errorf("Mail pricing repo: refresh Zone statuses: %w", err)
	}
	return nil
}

func (r *mailPricingRepository) ClaimMailPricingOutbox(ctx context.Context, claimToken uuid.UUID, leaseUntil time.Time, limit int) ([]*entity.PricingOutboxRow, error) {
	rows, err := r.db.Query(ctx, `WITH candidates AS (
		SELECT id FROM billing.pricing_outbox WHERE module_code='mail' AND published_at IS NULL AND available_at<=NOW() AND (lease_until IS NULL OR lease_until<NOW()) ORDER BY occurred_at,id FOR UPDATE SKIP LOCKED LIMIT $3
	), claimed AS (
	...
	if err != nil {
		return nil, fmt.Errorf("Mail pricing repo: claim outbox: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.ID, &row.PricingScheduleID, &row.VersionID, &row.VersionNumber, &row.ChargeKindCode, &row.EffectiveFrom, &row.Checksum, &row.OccurredAt, &row.ClaimToken, &row.RetryCount); err != nil {
			return nil, fmt.Errorf("Mail pricing repo: scan claimed outbox: %w", err)
		}
		batch = append(batch, &row)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("Mail pricing repo: iterate claimed outbox: %w", err)
	}
	return batch, nil
}

func (r *mailPricingRepository) MarkMailPricingOutboxPublished(ctx context.Context, id, claimToken uuid.UUID) error {
	...
	if err != nil {
		return fmt.Errorf("Mail pricing repo: mark outbox published: %w", err)
	}
	if result.RowsAffected() != 1 {
		return fmt.Errorf("Mail pricing repo: outbox claim lost for %s", id)
	}
	return nil
}

func (r *mailPricingRepository) RetryMailPricingOutbox(ctx context.Context, id, claimToken uuid.UUID, lastError string, availableAt time.Time) error {
	...
	if err != nil {
		return fmt.Errorf("Mail pricing repo: retry outbox: %w", err)
	}
	return nil
}
