...
func (r *walletAdmissionOutboxRepository) ClaimUnpublishedWalletAdmissionBatch(ctx context.Context, limit int, claimToken uuid.UUID) ([]*entity.WalletAdmissionOutboxRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.EventID, &row.OwnerID, &ownerType, &row.WalletVersion, &row.AdmissionMode, &row.RestrictionReason, &row.EffectiveAt, &row.ValidUntil, &row.OccurredAt, &row.ClaimToken); err != nil {
