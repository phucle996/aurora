...
// [COMMENT]: NewLifecycleRepository khởi tạo instance repository thực thi SQL cho sự kiện vòng đời.
func NewLifecycleRepository(db *pgxpool.Pool) billingRepoInterface.LifecycleRepository {
...
func (r *lifecycleRepository) ApplyLifecycleEvent(ctx context.Context, event *entity.LifecycleEvent) error {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		...
		log.Printf("[LifecycleRepo] Event %s đã tồn tại trong inbox. Bỏ qua xử lý trùng lặp.", event.EventID)
		return nil
	} else if err != nil {
	...
	// 3. Kiểm tra head version hiện tại trong resource_lifecycle_head
	var lastVersion int64
	var currentState string
	err = tx.QueryRow(ctx,
		`SELECT last_source_version, lifecycle_state FROM billing.resource_lifecycle_head WHERE resource_id = $1`,
		event.ResourceID,
	...
	if err == nil {
		...
		if event.SourceVersion <= lastVersion {
			log.Printf("[LifecycleRepo] Bỏ qua out-of-order event. Resource %s current head version=%d, event version=%d", event.ResourceID, lastVersion, event.SourceVersion)
	...
	case entity.ResourceLifecycleEventCreated:
		projID := uuid.New()
		_, err = tx.Exec(ctx, `
		...
		_, err = tx.Exec(ctx, `
			INSERT INTO billing.resource_lifecycle_head (resource_id, last_source_version, lifecycle_state, updated_at)
			VALUES ($1, $2, 'ACTIVE', NOW())
			ON CONFLICT (resource_id) DO UPDATE
			SET last_source_version = EXCLUDED.last_source_version, lifecycle_state = 'ACTIVE', updated_at = NOW()
		`, event.ResourceID, event.SourceVersion)
		if err != nil {
			return fmt.Errorf("lifecycle repo: upsert lifecycle head ACTIVE failed: %w", err)
		}

	case entity.ResourceLifecycleEventDeleted:
		_, err = tx.Exec(ctx, `
		...
		_, err = tx.Exec(ctx, `
			...
			ON CONFLICT (resource_id) DO UPDATE
			SET last_source_version = EXCLUDED.last_source_version, lifecycle_state = 'DELETED', updated_at = NOW()
		`, event.ResourceID, event.SourceVersion)
		if err != nil {
