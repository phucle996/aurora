...
const (
	personalAccountActivatedEventType = "iam.personal_account.activated.v1"
	personalAccountActivatedStream    = "iam:personal-account:activated:v1"
	personalAccountActivatedGroup     = "cost-personal-account-wallet-adapter-v1"
	personalAccountActivatedDLQ       = "billing:wallet:personal:provision-dlq"
	personalWalletReclaimIdle         = 30 * time.Second
	personalWalletMaxDeliveries       = 25
)

// PersonalAccountActivatedConsumer adapts an IAM-owned fact into Cost's wallet projection.
// PostgreSQL inbox + wallet mutation remains the idempotency and apply boundary.
type PersonalAccountActivatedConsumer struct {
...
func NewPersonalAccountActivatedConsumer(
	sharedRedis *goredis.Client,
	service billingSvcInterface.PersonalAccountService,
) (*PersonalAccountActivatedConsumer, error) {
	if sharedRedis == nil || service == nil {
		return nil, errors.New("personal account activation adapter requires Shared Redis and PersonalAccountService")
	}
	hostname, err := os.Hostname()
	if err != nil || strings.TrimSpace(hostname) == "" {
		hostname = "cost-manager"
	}
	return &PersonalAccountActivatedConsumer{
...
func (s *PersonalAccountActivatedConsumer) Start() error {
	if s == nil {
	...
	if err := s.sharedRedis.XGroupCreateMkStream(
		ctx,
		personalAccountActivatedStream,
		personalAccountActivatedGroup,
		"0",
...
func (s *PersonalAccountActivatedConsumer) run(ctx context.Context) {
	defer close(s.done)
	...
	for {
		...
		claimed, _, err := s.sharedRedis.XAutoClaim(ctx, &goredis.XAutoClaimArgs{
			Stream:   personalAccountActivatedStream,
			Group:    personalAccountActivatedGroup,
			Consumer: s.consumer,
		...
		streams, err := s.sharedRedis.XReadGroup(ctx, &goredis.XReadGroupArgs{
			Group:    personalAccountActivatedGroup,
			Consumer: s.consumer,
			Streams:  []string{personalAccountActivatedStream, ">"},
			Count:    32,
			Block:    5 * time.Second,
		}).Result()
		if err != nil {
...
func (s *PersonalAccountActivatedConsumer) process(ctx context.Context, message goredis.XMessage) {
	...
	wire := &iamlifecyclev1.PersonalAccountActivatedV1{}
	...
	ownerID, ownerErr := uuid.FromBytes(wire.GetUserId())
	...
	if eventErr != nil || protoErr != nil || wireEventErr != nil || ownerErr != nil ||
		...
		eventType != personalAccountActivatedEventType ||
	...
	if _, err := s.sharedRedis.TxPipelined(ctx, func(pipe goredis.Pipeliner) error {
		pipe.XAck(ctx, personalAccountActivatedStream, personalAccountActivatedGroup, message.ID)
		pipe.XDel(ctx, personalAccountActivatedStream, message.ID)
...
func (s *PersonalAccountActivatedConsumer) deadLetter(
	ctx context.Context,
	...
	if _, err := s.sharedRedis.TxPipelined(ctx, func(pipe goredis.Pipeliner) error {
		pipe.XAdd(ctx, &goredis.XAddArgs{
			Stream: personalAccountActivatedDLQ,
...
func (s *PersonalAccountActivatedConsumer) Stop() {
	if s == nil {
		return
	}
	s.once.Do(func() {
