...
impl ValidatedJob {
    ...
    fn from_command(
        ...
        match (workload, command.source_domain.as_str()) {
            ...
            ("managed_service", "MANAGED_SERVICE") => false,
            ("managed_service", _) => {
                return Err(JobValidationError::new(
...
mod tests {
    ...
    #[test]
    fn validation_accepts_protected_managed_service_route_for_p06_executor() {
        let keyring = PayloadKeyring::for_test();
        let mut command = command(&keyring);
        command.job_topic = "managed_service.instance.execute".to_string();
        command.source_domain = "MANAGED_SERVICE".to_string();
        protect(&mut command, &keyring);
        let job = ValidatedJob::from_command(command, ZONE_A, 5, &keyring)
            .expect("managed service route should reach the P06 executor");
        assert_eq!(job.job_topic, "managed_service.instance.execute");
        assert_eq!(job.source_domain, "MANAGED_SERVICE");
    }

    #[test]
    fn managed_service_cross_zone_is_rejected_before_hpke_open() {
        let keyring = PayloadKeyring::for_test();
        let mut command = command(&keyring);
        command.job_topic = "managed_service.instance.execute".to_string();
        command.source_domain = "MANAGED_SERVICE".to_string();
