...
///
/// 📌 VAI TRÒ (ROLE):
///   - Quản lý vòng lặp poll Proxmox Cluster API mỗi 15 giây.
...
impl HypervisorMonitor {
    ...
    pub fn start(config: Arc<Config>, redis_internal_zone: Arc<RedisClientManager>) {
        ...
        tokio::spawn(async move {
            ...
            // Xây dựng proxmox client tái sử dụng
            let proxmox_client = ProxmoxClient::new(&config);
            ...
            loop {
                ...
                let poll_result = proxmox_client.fetch_nodes().await;
