...
impl SessionManager {
    ...
    // Trả về danh sách client_device_ids của các session bị evict khi vượt quá USER_DEVICE_CAP.
    pub async fn register_session(
        ...
        // [COMMENT]: Kiểm tra số lượng session sau khi thêm. Nếu vượt quá USER_DEVICE_CAP,
        // lấy toàn bộ session của user, sắp xếp theo lsa tăng dần (cũ nhất trước), và evict bớt.
        ...
        // [COMMENT]: Lấy toàn bộ session key trong user index
        ...
        // [COMMENT]: Fetch toàn bộ dữ liệu session theo pipeline
        ...
        // [COMMENT]: Parse và ghép cặp (redis_key, tdid, lsa) để sort theo lsa
        ...
        // [COMMENT]: Sort ascending bởi lsa — session cũ nhất (ít hoạt động nhất) xếp đầu để evict trước
        ...
        // [COMMENT]: Xoá các session bị evict khỏi Redis (DEL key + SREM từ user index)
        ...
        // [COMMENT]: Thu thập client_device_ids bị evict để báo về Control Plane
