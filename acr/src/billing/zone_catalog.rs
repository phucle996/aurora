// ======================================================================================================
// 📂 billing/zone_catalog.rs — Billing Zone Catalog Handler (GET /api/v1/billing/zones/catalog)
// ======================================================================================================
...
/// [COMMENT]: Intercept GET /api/v1/billing/zones/catalog dành cho Billing Auditor domain.
pub async fn handle_billing_zone_catalog(
    ...
    if !(method == "GET" && path.starts_with("/api/v1/billing/zones/catalog")) {
        return None;
    }
    ...
    let json_body = match serde_json::to_string(&catalog) {
        ...
        Err(e) => {
            Logger::sys_error(
                ...
                "Failed to serialize billing zone catalog",
                &e.to_string(),
            );
    ...
    response.set_status(tonic::Status::unauthenticated(
        "Billing zone catalog local intercept",
    ));
    response.set_http_response(denied_builder);
